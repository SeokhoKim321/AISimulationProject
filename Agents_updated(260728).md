...
- 현재 코드 버전: `260728_audio_anchored_spawn_v27`
- 보관 스냅샷: `FLYWITHLUA_STUDY_INTEGRATED_260728_audio_anchored_spawn_v27.lua`
...
- v26은 trial 시작 `3초` 후 표준 계기 유지 WAV를 자동 재생하고 `TASK_COMMAND_AUDIO_START/END`를 기록한다.
...
- Lua v27은 participant trial 중 한 줄의 `TRIAL IDLE/RESET/ACTIVE/END` 상태만 표시한다. 종료가 guard에 거부되면 `TRIAL IN PROGRESS - END NOT READY`를 2.5초 동안 표시한다. countdown, intruder/scenario 상태, debug 좌표·거리와 `TRAFFIC ALERT`/`CHECK OUTSIDE`는 숨긴다.
- v27 표준 음성은 `Microsoft Zira` rate 1로 생성한 6.493초 WAV이며, `Cessna zero one, continue straight in, maintain seven zero knots, track runway centerline.`을 재생한다.
...
- Java response detector는 아직 `ADVISORY_SHOWN`을 기준으로 동작하므로 다음 구현에서 `INTRUDER_SPAWNED` 기준으로 변경해야 한다.
- v27 random crossing은 `TASK_COMMAND_AUDIO_START` 직후 `U(3,10초)`를 독립 추출하고 그 시간이 지난 뒤 intruder를 생성한다.
