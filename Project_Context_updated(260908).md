...
## 2026-08-28 연구실 기술 파일럿 및 논문 데이터 구성

2026-08-25~26에 동일 v39 자동 6-trial 절차로 연구실 참가자 7명과 experienced
operator reference 1명을 수집했다. 원본 X-Plane/Tobii CSV는 `logs/`에 보존하며 Git과
공개 보고서에는 포함하지 않는다. 보고용 participant id는 `P01~P07`로 익명화한다.

Primary dataset:

- complete laboratory participants: `6`
- complete trials: `36`
- direction balance: FRONT/LEFT/RIGHT 각 `12`
- participant-equal trial gaze validity: 약 `96.3%`
- response detection: `31/36 = 86.1%`
- detected visual-opportunity-to-response: mean `3.135 s`, median `2.138 s`
- full-trial eligible bins/transitions/self/switches: `6077/5942/4745/1197`
- full-trial self-transition rate: `79.9%`
- participant-equal switch frequency: 약 `59.0/min`
- participant operational-external range: `4.0~71.7%`
- speed stable/forced: `16/20`; target 110 KIAS는 `7/7` forced

부분 세션 1명은 수면 충돌 전에 완료된 4 trial을 그대로 보존한다. 이 세션은 시작
고도도 다른 참가자보다 낮았으므로 사고결과를 숙련도 차이로 해석하지 않고 primary
집단 Matrix에서 제외한다. 포함 sensitivity에서 전체 matrix 평균 변화는 작았으나 sparse
row는 변화가 커질 수 있다.

Experienced operator reference는 2026-08-26 최신 세션
`session_main_KIM_SEOKHO_20260826_164645`만 본문값으로 사용한다.

- clean trial: `6/6`
- raw/trial gaze validity: `88.5/91.5%`
- operational-external: `48.4%`
- visual-opportunity-to-response: 전체 mean/median `2.467/2.442 s`
- opportunity 당시 active control 제외: `n=3`, mean `3.834 s`
- full-trial instrument/panel-other/external state: `43.3/2.6/54.2%`
- self-transition/switch frequency: `74.0% / 74.4 per min`
- mean/median minimum horizontal distance: `35.0/25.8 m`
- speed stable/forced: `3/3`

2026-08-12 self 세션은 최신 reference와 full-trial instrument 비율 `43.3%`,
self-transition 약 `74%`, switch 약 `73~74/min`으로 유사했다. 반복 재현성 확인용으로
보존하되 집단 평균이나 최신 reference 본문값에는 합치지 않는다.

논문 방향:

- APISAT: event-aligned X-Plane/Java/Tobii pipeline, 20 px geometry opportunity,
  grid/dynamic AOI, phase-specific DTMC/next-switch의 technical pilot validation
- KAI 항공우주논문상: `미래SW` 분야의 simulator-independent training/debriefing
  software와 MUH/마린온 시뮬레이터 adapter 후속 적용 개념
- 현재 결과로 숙련 조종사 대 비숙련자 우열, 실제 intruder 인지시간, direction/speed의
  인과효과를 주장하지 않는다.
...
## 2026-09-01 제3회 해병항공 전투발전 워크숍 발표자료

- 2026-09-22 워크숍 발표 제목은 `AI 데이터를 활용한 조종능력 향상 방안`이다.
- 기준 논문은 `2026 APISAT/APISAT-2026_FullPaper_KimSeokho_merged.pdf`이며,
  논문의 구현·기술검증 결과와 군 시뮬레이터 후속 이식 제안을 분리해 구성했다.
- 발표자료는 본 발표 17장과 부록 3장, 총 20장이다. 발표자 노트 기준 본 발표 시간은
  약 `19분 55초`다.
- 산출물은 `2026 해병항공 전투발전 워크숍/` 아래 PPTX와 동명 PDF로 저장했다.
- 핵심 메시지는 AI 자동채점이 아니라 시선·조종·상황 event를 동기화하여 교관의
  설명 가능한 디브리핑과 개인 반복훈련을 지원하는 것이다.
- 군 적용은 X-Plane 반입이 아니라 인가 시뮬레이터의 상태·입력·event interface를
  공통 data contract로 변환하는 adapter와 폐쇄망 분석 노드 도입으로 제안한다.
- 함상 이·착함/저시정, 해상·저고도 공중기동, 야간·악천후·비상절차,
