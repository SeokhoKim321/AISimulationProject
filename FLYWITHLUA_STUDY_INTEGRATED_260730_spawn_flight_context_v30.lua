...
local STUDY_SCRIPT_VERSION = "260730_spawn_flight_context_v30"
local PARTICIPANT_NOTICE_DURATION_S = 2.5
local TASK_AUDIO_PROTOCOL = "audio_task_v29"
...
function study_start_trial()
    ...
    study_record_event(
        ...
        study_trial_detail(
            string.format(
                "protocol=%s;target_speed_kias=%d;task_audio_file=%s;task_audio_baseline_s=%.3f;sound_master=%.3f;sound_interior=%.3f;sound_engine=%.3f;sound_prop=%.3f;sound_enviro=%.3f;sound_radio=%.3f",
                TASK_AUDIO_PROTOCOL,
                task_audio_current_option.target_speed_kias,
                task_audio_current_option.file_name,
...
function study_start_intruder_crossing()
    ...
    crossing_spawn_event_detail = study_trial_detail(
        string.format(
            "scenario=random_final_approach_crossing;profile=realistic_ttc_v13_near_path_intrusion;spawn_phase=stabilization_start;delay_anchor=task_audio_start;planned_audio_to_spawn_s=%.3f;actual_audio_to_spawn_s=%.3f;approach=%s;side_sign=%.0f;planned_move_to_path_s=%.3f;ownship_arrival_to_path_s=%.3f;path_lead_time_s=%.3f;target_ahead_m=%.3f;start_distance_m=%.3f;spawn_extra_distance_m=%.3f;base_speed_mps=%.3f;effective_speed_mps=%.3f;vertical_speed_mps=%.3f;vertical_match_gain=%.3f;stabilization_s=%.3f;heading_deg=%.3f",
            crossing_planned_audio_to_spawn_s,
