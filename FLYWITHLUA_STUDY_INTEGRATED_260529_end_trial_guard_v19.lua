...
local TARGET_HOST = "100.64.0.129"
local TARGET_PORT = 9100
local STUDY_SCRIPT_VERSION = "260529_end_trial_guard_v19"
...
crossing_delay_min_s = 6.0
crossing_delay_max_s = 14.0
...
function study_build_intruder_line()
    ...
    return string.format(
        "INTRUDER,%d,%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f",
        trial_id,
        sample_index,
        running_time_sec,
...
function study_start_trial()
    ...
    study_record_event("TRIAL_START", study_trial_detail())
    study_arm_random_crossing()
end

function study_trial_can_end()
    return not intruder_headon_active
        and not intruder_crossing_active
...
function study_arm_random_crossing()
    if not trial_active then
        study_status_text = "START TRIAL FIRST"
    ...
    local delay_s = crossing_delay_min_s + math.random() * delay_range
    crossing_trigger_time_s = running_time_sec + delay_s
    random_crossing_armed = true
    intruder_crossing_active = false
    crossing_min_distance_reported = false

    study_status_text = string.format("RANDOM CROSSING ARMED: %.1fs", delay_s)
    study_record_event("FINAL_APPROACH_GATE_ENTERED", study_trial_detail("gate=trial_start_placeholder"))
    study_record_event("SCENARIO_SELECTED", study_trial_detail(string.format("scenario=random_final_approach_crossing;delay_s=%.3f", delay_s)))
end
...
function study_start_intruder_crossing()
    ...
    crossing_spawn_event_detail = study_trial_detail(string.format("scenario=random_final_approach_crossing;profile=realistic_ttc_v13_near_path_intrusion;approach=%s;side_sign=%.0f;planned_move_to_path_s=%.3f;ownship_arrival_to_path_s=%.3f;path_lead_time_s=%.3f;target_ahead_m=%.3f;start_distance_m=%.3f;spawn_extra_distance_m=%.3f;base_speed_mps=%.3f;effective_speed_mps=%.3f;vertical_speed_mps=%.3f;vertical_match_gain=%.3f;stabilization_s=%.3f;heading_deg=%.3f", crossing_approach_mode, intruder_side_sign, crossing_time_to_conflict_s, target_prediction_time_s, crossing_path_lead_time_s, predicted_distance, crossing_start_distance_m, crossing_spawn_extra_distance_m, crossing_speed_mps, crossing_effective_speed_mps, crossing_vertical_speed_mps, crossing_vertical_match_gain, crossing_stabilization_duration_s, intruder_heading_deg))
    logMsg("[StudyIntegrated] crossing intruder stabilizing")
end
...
function study_update_random_crossing()
    ...
    if crossing_stabilizing then
        ...
        crossing_stabilizing = false
        intruder_start_time_s = running_time_sec
        if not crossing_spawn_event_recorded then
