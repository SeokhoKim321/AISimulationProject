...
- Lua/분석 v27 audio 기준 `3~10초` spawn 구현 완료
- spawn 기준 Java response 구현 보완 대기

Lua는 v27로 갱신되어 최소 trial 상태 정책을 유지하면서 trial 시작 3초 뒤
표준 계기 유지 음성을 자동 재생하고, audio start를 기준으로 독립적인
`U(3,10초)` 뒤 intruder를 생성한다. 내부 logging/guard는 유지하며,
다음은 실제 v27 timing pilot과 Java response 기준 변경이다.
...
- `Agents_updated(260728).md`
- `Project_Context_updated(260728).md`
- `Worklog_updated(260728).md`
...
- v27 delay는 audio start 이후 독립적인 `U(3,10초)`이다.
- v27은 trial 상태와 2.5초 end 거부 알림만 남기고 audio countdown/debug/advisory는 participant 화면에서 숨긴다.
- trial 시작 3초 뒤 고정 WAV를 재생하고 `TASK_COMMAND_AUDIO_START/END`를 기록한다.
