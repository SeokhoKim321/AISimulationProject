...
- 현재 코드 버전: `260811_intruder_ai_autopilot_override_v39`
- 보관 스냅샷: `FLYWITHLUA_STUDY_INTEGRATED_260811_intruder_ai_autopilot_override_v39.lua`
- task protocol: `audio_task_v39`

### v39 slot-1 AI autopilot override light diagnostic

- v38 physical test에서 화면상 불빛이 계속 보였다.
- v38 log는 spawn 직후 모든 light readback이 0이었지만 다음 frame마다 X-Plane이
  landing/taxi를 `1/1`로 복구했음을 보였다. 첫 opportunity까지
  `lights_reenabled_count=333`이었다. beacon/nav/strobe는 계속 0이었다.
- v39은 intruder active 동안
  `sim/operation/override/override_plane_ai_autopilot[1]=1`을 유지하고 종료/중단/reload
  초기화 시 0으로 복원한다. slot 1 위치와 속도는 기존 Lua가 계속 직접 갱신한다.
- event detail에 `ai_autopilot_override_write_ok`,
  `ai_autopilot_override_readback_ok`, `ai_autopilot_override_value`를 추가한다.
- v39 single-trial physical test에서 참가자가 불빛 소멸을 육안 확인했다. 로그상
  override write/readback/value는 `true/true/1`, 다섯 light readback은 모두 0,
  `lights_reenabled_count=0`이었다.
- LEFT trial은 spawn-to-opportunity `5.452 s`, `MIN_DISTANCE_REACHED`, manual
  `TRIAL_END`까지 정상 진행되어 intruder 이동/종료 흐름도 유지됐다.
- 후속 balanced 6-trial physical test도 PASS했다. 방향표는
  `LEFT|RIGHT|FRONT|RIGHT|FRONT|LEFT`로 각 방향 2회였고, 전 trial에서 override=1,
  다섯 light OFF, `lights_reenabled_count=0`이었다.
- spawn/opportunity/min-distance/trial-end는 각각 `6/6`, session complete는 `1/1`,
  reenabled marker와 Lua error는 `0`이었다. spawn-to-opportunity는
  `5.295~10.814 s`로 교정 목표 `5~11 s`를 모두 만족했다.
- v39 조명 억제 및 intruder movement는 engineering PASS이며 현재 권장 활성본이다.
- 기준본, v39 snapshot, 실제 Scripts 실행본 SHA-256은
  `6556B91F9E2AA568BE0C573261F1AE99BB09602505DB395206D36E1013F6BFE1`로 일치한다.

### 2026-08-12 v39 integrated Java/Tobii validation

- integrated session은 `session_v39_integrated_20260812_155103`이고 gaze session은
  `session_xplane_tobii_20260812_155022`다.
- 원본 row는 STATE `3247`, INTRUDER `3247`, EVENT `103`, gaze `30140`이며
  `RECEIVER_STOP`과 `SESSION_STOP`까지 정상 기록됐다.
- Java session analysis는 LEFT/RIGHT/FRONT 각 2회의 총 6개 trial을 모두 clean으로
  판정했다. spawn, 20 px visual opportunity, response start/end, automatic trial end는
  모두 `6/6`이다.
- 생성 및 opportunity event 모두 slot-1 AI autopilot override readback이 1이고
  `lights_all_off=true`였다. positive light reenabled count는 0이므로 v39 조명 억제는
  Java/Tobii 동시 기록 조건에서도 PASS다.
- spawn-to-opportunity는 `5.618~10.921 s`로 6개 모두 교정 범위 `5~11 s`를 만족했다.
  initial span은 `8.050~13.701 px`이며 provisional `7~13 px` 범위는 `5/6`이다.
- raw gaze validity는 `21207/30140 = 70.4%`, trial별 validity는
  `83.8~92.6%`, trial 동일가중 평균은 `87.6%`다.
- visual-opportunity-to-response는 `1.631~10.199 s`, 평균 `4.870 s`, 중앙값
  `3.838 s`다. trial 5는 opportunity 당시 control active이므로 해당 latency는 주의
  표식을 유지한다.
- speed spawn gate는 stable `4/6`, forced timeout `2/6`이다. trial 1은 목표오차
  `+14.037 KIAS`이므로 본분석 제외 후보이고, trial 6은 오차 `+3.745 KIAS`지만
  dwell이 `1.357 s`로 1.5 s 기준에 미달해 forced로 구분한다.
- trial 구간의 operational-external 평균은 `49.3%`다. 이는 외부환경 시선 proxy이며
  intruder 인지나 획득의 확정값으로 해석하지 않는다.
- recording clean과 실험조건 적합성을 분리한다. 이 세션은 engineering recording
  PASS지만 primary 연구분석은 stable trial을 기본으로 하고 forced trial 포함 결과를
  민감도 분석으로 병기한다.

### 2026-08-19 external gaze context classifier

- 외부시선의 공간·객체·의미 층을 분리하는 사후분석 도구
  `tools/tobii/classify_xplane_external_gaze.py`를 추가했다.
- 입력은 원본 STATE/INTRUDER/EVENT와 기존 `*_gaze_aoi.csv`다. 원본 CSV와 UDP
  packet schema는 변경하지 않는다.
- spatial layer는 각 1920 px monitor를 `6 x 3` grid로 나누고
  `LEFT/CENTER/RIGHT_RnCn` cell을 보존한다.
- object layer는 각 gaze timestamp에 가장 가까운 X-Plane state와 같은 sample index의
  intruder를 결합한다. Lua v39과 같은 `5760 x 1080`, horizontal FOV `122 deg`,
  wingspan `11.0 m`, length `8.3 m`, roll ignored projection을 재계산한다.
- runway object layer는 `resources/rksi_runway34_geometry_260819.csv`의 X-Plane 11
  `apt.dat` runway 34 양 끝점·폭·고도를 local coordinates로 변환해 매 시각 투영한다.
- semantic layer는 `INTRUDER_SEARCH_OR_TRACKING`, `RUNWAY_GUIDANCE`,
  `OTHER_EXTERNAL`, `UNRESOLVED`이며 기본 dwell은 `200 ms`다.
- intruder evidence는 direct dynamic AOI overlap, same-grid search proxy,
  spawn 이후 measured side-direction match proxy를 별도 필드로 보존한다.
- 기본 intruder/runway margin은 각각 `45 px`인 engineering candidate다. priority는
  intruder dynamic AOI, runway dynamic AOI, intruder grid, measured side match,
  residual other 순이다.
- `INTRUDER_SEARCH_OR_TRACKING`은 실제 발견·인지·의도의 확정값이 아니다. 논문에서는
  spatial/object-context operational evidence로만 표현한다.
- v39 integrated session에서 Lua event 대비 사후 projection median absolute error는
  x `0.083 px`, y `0.363 px`, span `0.026 px`로 PASS했다.
- v39 trial 내부 operational-external `5944` rows의 기본 분류는 intruder `24.9%`,
  runway `5.0%`, other `47.9%`, unresolved `22.3%`다. intruder 근거는 direct
  dynamic overlap `879`, grid proxy `438`, measured side proxy `161` rows다.
- v37 actual-session backward check도 projection median absolute error x `0.233 px`,
  y `0.724 px`, span `0.025 px`로 PASS했다.
- `30/45/60 px` sensitivity에서 분류 비율이 변하므로 `45 px`는 아직 본실험 동결값이
  아니다. 화면 녹화 또는 통제된 표적 주시 수동검증 후 확정한다.
- 실행 절차와 출력 계약은 `TOBII_EXTERNAL_GAZE_CLASSIFICATION_260819.md`를 따른다.

### 2026-08-24 AOI transition matrix and lab participant protocol

- 벤치마크 논문 `CognitiveAgentEvaluationforSyntheticPilotTraining.pdf`의 attention
  distribution model은 source attention zone `i`에서 다음 zone `j`로 갈 조건부확률
  `P(s_(t+1)=j | s_t=i)`을 행으로 정규화한 DTMC다. 대각선 self-transition은 같은
  AOI에 계속 머무는 attention persistence를 나타낸다.
- 프로젝트용 도구 `tools/tobii/analyze_aoi_transition_matrix.py`와 synthetic smoke
  `tools/tobii/analyze_aoi_transition_matrix_smoke_test.py`를 추가했다.
- 입력은 외부 context classifier의 `*_external_context_gaze.csv`와 같은 세션의
  X-Plane `*_events.csv`다. 원본 gaze/X-Plane CSV schema는 변경하지 않는다.
- 기본 상태 단위는 `200 ms` bin의 dominant eligible AOI다. 이것은 검증된 fixation
  detector 출력이 아니므로 fixation으로 부르지 않는다. invalid/unresolved bin은
  전이 chain을 끊으며 앞뒤 상태를 직접 연결하지 않는다.
- AOI state는 6개 계기, `PANEL_OTHER`, `INTRUDER_AOI`, `RUNWAY_AOI`,
  `OTHER_EXTERNAL`이다. intruder/runway state는 operational object-overlap evidence이며
  실제 인지·의도의 확정값이 아니다.
- 논문형 DTMC는 self-transition을 포함하고, 설명용 next-switch matrix는
  self-transition을 제외한다. 두 행렬 모두 count/probability CSV와 SVG를 출력한다.
- phase는 `FULL_TRIAL`, `BEFORE_SPAWN`, `SPAWN_TO_OPPORTUNITY`,
  `AFTER_OPPORTUNITY` 네 가지다. phase/trial 경계를 넘어 전이를 만들지 않는다.
- 여러 참가자 분석은 participant별 matrix, pooled transition matrix,
  participant-equal mean probability matrix를 모두 보존한다. 기본 SVG는 유효 표본 수가
  많은 한 사람이 집단값을 지배하지 않도록 participant-equal mean을 사용한다.
- v39 integrated single-participant 재분석은 binned states `1745`, DTMC transitions
  `1617`, AOI switches `420`이다. `FULL_TRIAL`은 eligible bins `873`, transitions
  `816`, self `603 (73.9%)`, switches `213`이다. probability row-sum과 SVG XML 검증은
  PASS했다.
- Tobii logger는 선택적인 `--participant-code`와 `--duration-s`를 받는다. participant
  code가 없으면 기존 파일명 규칙을 유지하므로 CSV schema와 과거 절차는 호환된다.
- 연구실 구성원 1인 1회는 각 participant당 자동 6-trial session 1개로 준비한다.
  LEFT/RIGHT/FRONT는 매 session 각 2회이며 순서는 balanced random이다.
- 학위논문·학술발표에 사용할 사람 데이터는 IRB 승인 또는 공식 면제 판단 이후에만
  수집한다. 승인 전 technical test를 나중에 main participant data로 전환하지 않는다.
  비조종 연구실 인원의 결과는 feasibility/novice 자료이지 pilot 대표값이 아니다.
- 전체 방법과 PowerShell 실행/후처리 절차는
  `AOI_TRANSITION_MATRIX_AND_LAB_PILOT_PROTOCOL_260824.md`를 따른다. 다중 session 입력은
  `resources/aoi_transition_session_manifest_template_260824.csv` 형식을 사용한다.

### v38 persistent intruder-lights-off diagnostic

- 참가자의 실제 화면 확인 결과, v37의 `lights=off`, `lights_write_ok=true`에도
  intruder 점멸등이 계속 보였다. 따라서 `lights_write_ok`만으로 실제 렌더링 소멸을
  입증하지 않는다.
- v38 당시 profile은 `visual_opportunity_proxy_v38`이었다. v38은 intruder가 active인 동안
  multiplayer slot 1의 beacon/nav/strobe/landing/taxi light DataRef를 매 frame 0으로
  강제한다.
- 각 write 뒤 DataRef를 다시 읽어 `lights_readback_ok`, `lights_all_off`, 개별
  `light_readback_*`, `lights_reenabled_count`를 event detail에 기록한다. 다음 frame
  전에 값이 다시 켜지면 최초 1회 `SCENARIO_MARKER`에
  `intruder_lights_reenabled_by_sim=true`를 기록한다.
- X-Plane 공식 지침상 AI aircraft 제어에 `override_planepath`를 사용하지 않는다.
  이 override는 AI aircraft를 정지시키거나 좌표 제어와 충돌할 수 있으므로 조명
  제거를 위해 추가하지 않았다.
- v38에서도 육안 점멸이 남고 readback이 모두 0이면 현재 AI aircraft 모델의
  object/aircraft 내장 light일 가능성이 높다. 다음 대안은 no-light AI aircraft model
  또는 연구용 custom object다.
- Java 21 compile, Java/Python smoke, v37 실제 session backward reanalysis는 PASS했다.
- 기준본, v38 snapshot, 실제 X-Plane Scripts 실행본 SHA-256은
  `59EF750687D4C400335C4632463C79D9089A258FDD2AA801CDDCB2D7326760DE`로 일치한다.
- 실제 Scripts 폴더의 활성 Lua는 `FLYWITHLUA_STUDY_INTEGRATED.lua` 1개다.
- v34 자동 6-trial 방향표는 1~3번과 4~6번을 각각 LEFT/RIGHT/FRONT 1회씩 포함하는 두 무작위 블록으로 생성하며, 3번과 4번도 같은 방향이 연속되지 않게 한다.
- 2026-08-04 X-Plane runtime 검증에서 v34가 오류 없이 로드됐고, 생성 방향표 `FRONT|LEFT|RIGHT|LEFT|RIGHT|FRONT`가 두 블록 균형·인접 비반복 조건을 모두 만족했다. 초기 대기 중 stop도 trial 생성 없이 정상 기록됐다.
- v36은 음성 지시 후 `6~10 s` 무작위 candidate 시각과 목표 `±5 KIAS` 범위의 `1.5 s` 연속 dwell을 모두 만족한 뒤 intruder를 생성한다. IAS rate는 생성 필수조건이 아니라 진단값이다.
- 속도가 안정되지 않아도 음성 시작 `14 s`에는 강제 생성한다. 무기한 대기는 허용하지 않으며 안정 생성/제한시간 생성을 event detail에서 구분한다.

현재 behavior-evidence 도구:

- classifier: `tools/tobii/classify_xplane_tobii_behavior.py`
- smoke test: `tools/tobii/classify_xplane_tobii_behavior_smoke_test.py`
- version: `260804_behavior_evidence_v1`
- 출력: `*_trial_behavior_summary.csv`
- 이 분류는 recording clean과 별개이며 `SPEED_TASK_CORRECTION`, `AVOIDANCE_CANDIDATE`, `AMBIGUOUS`, `NO_VALID_RESPONSE`를 산출한다.
- `AVOIDANCE_CANDIDATE`는 조종사 의도의 확정값이 아니라 3-D projected CPA 개선, response 전 operational-external evidence, opportunity 당시 비활성 조작이 함께 있는 보수적 후보값이다.
- operational-external은 intruder acquisition을 뜻하지 않으며 CPA 개선만으로도 회피 의도를 확정하지 않는다.
...
- crossing intruder는 left/right/front 접근을 세션 내 균형 배정하고, ownship 예상 경로 근처를 통과하도록 배치된다.
...
- Java는 v33/v34에서 `INTRUDER_VISUAL_OPPORTUNITY_ONSET` 전 1초 입력 baseline 대비 지속 변화를 이용해 `PILOT_RESPONSE_START`, `PILOT_RESPONSE_END`를 자동 기록한다. v32는 `INTRUDER_VISUALLY_DETECTABLE`, v31 이하는 `INTRUDER_SPAWNED` anchor를 유지한다.
...
- 상태·intruder·event·Tobii trial summary를 결합해 속도과제 조작과 회피후보를 분리하는 오프라인 behavior-evidence classifier가 구현되어 있다.
...
- 위 항목은 v32 당시 검증 상태다. v33에서 20 px는 보조 진단값으로
  낮아졌으며 아래 v33 물리 검증 절차가 현재 기준이다.

## 2026-08-04 v33 light-assisted visual-opportunity rule

- 현재 활성 Lua는 `260804_visual_opportunity_bearing_v33`, protocol은
  `audio_task_v33`이다.
- crossing intruder 최초 상대방위는 FRONT `0°`, LEFT `-32°`, RIGHT `+32°`로
  고정한다. 5760x1080, 수평 FOV 122° 기준 좌우 위치는 각각 약
  `x=1882 px`, `x=3878 px`로 중앙 모니터 경계에서 인접 모니터 쪽으로 약
  `38 px` 들어간다.
- intruder는 기존 수직 교차 배치 대신 지정 상대방위 ray 위에서 시작해
  기존 예상 교차지점으로 수렴하도록 배치한다. 비현실적인 유효속도를
  만들던 추가 `500 m` path extension은 제거했고 기본 속도 `58 m/s`를
  유지한다.
- spawn 시 beacon/nav/strobe light를 켜고 landing/taxi light는 끈다. retire
  시에는 모든 해당 light를 끈다. 실제 X-Plane에서 dataref write 성공과
  점멸 표시를 반드시 확인한다.
- 초기 projection이 화면 안에 있고 추정 수평 span이 `5 px` 이상이며 light
  write가 성공하면 `INTRUDER_VISUAL_OPPORTUNITY_ONSET`을 기록한다.
- 이 event의 method는 `light_assisted_operational_proxy_v1`이고 detail에는
  `operational_proxy=true`, `recognition_claim=false`를 명시한다. 이는
  조종사가 실제로 항공기를 인지한 시점이 아니라, 조명된 표적을 관측할
  수 있게 만든 실험상 기준시점이다.
- Java v33 response detector의 primary anchor는
  `INTRUDER_VISUAL_OPPORTUNITY_ONSET`이다. 기존 `INTRUDER_VISUALLY_DETECTABLE`
  20 px event는 보조 진단값으로 유지하며 v33 baseline을 덮어쓰지 않는다.
- v33 strict recording completeness는 opportunity event가 정확히 한 번이고
  `INTRUDER_SPAWNED -> INTRUDER_VISUAL_OPPORTUNITY_ONSET -> TRIAL_END` 순서인지를
  확인한다. 20 px event는 v33 clean 필수조건이 아니다.
- Java session/batch export와 Tobii analyzer v9는 opportunity geometry,
  `spawn->opportunity`, `opportunity->response`, opportunity-relative external
  view/AOI 지표를 저장한다.
- `visualize_tobii_gaze.py`의 기본 anchor도 opportunity event로 변경됐다.
  과거 분석은 `--event INTRUDER_VISUALLY_DETECTABLE`,
  `--event INTRUDER_SPAWNED`, `--event ADVISORY_SHOWN`을 명시해 재현한다.
- 정적 형상 검증에서 90~110 kt ownship 범위의 초기 거리/추정 span은 FRONT
  약 `1.86~2.23 km / 7.9~9.4 px`, LEFT/RIGHT 약
  `1.56~1.90 km / 12.3~14.7 px`였다. 20 px event는 모든 궤적에서 반드시
  발생하지 않으므로 primary outcome으로 사용하지 않는다.
- Java 21 compile, Java auto-event/speed-task smoke, session/batch export,
  Python 3.10 compile, v33 Tobii merge smoke, v33 opportunity 누락 clean 거부가
  모두 통과했다.
- 다음 물리 검증은 FlyWithLua reload 후 FRONT/LEFT/RIGHT의 실제 생성 위치,
  beacon/nav/strobe 표시, event 기록과 6-trial 자동 진행을 확인하는 것이다.

## 2026-08-04 v33 physical validation result

X-Plane AI aircraft operating rule:

- 본 실험에서는 X-Plane AI aircraft를 intruder 표시용 Cessna 172 한 대만
  둔다. 확인값은 `Freeflight.prf: _num_other_aircraft 1`,
  `X-Plane.prf: _num_acf 2`(ownship 포함)이다.
- JoinFS는 실험 중 비활성화한다. 불필요한 multiplayer/traffic source는
  외부시야 자극과 multiplayer slot ownership의 교란요인이다.
- 첫 physical run `session_visual_opportunity_v33_20260804_132756`은 기존
  AI aircraft 19대 상태에서 3개 clean trial 뒤 X-Plane이 종료됐다.
  X-Plane `Log.txt`의 직접 원인은 내장 ATC의
  `ETA is wrong / GetGeoPositionAtT / ATC_aircraft.cpp:147` assert였으며,
  Lua/FlyWithLua load error가 아니었다.
- AI를 한 대로 줄인 retry에서는 같은 crash가 재발하지 않았다.

Successful retry session:

- session: `session_visual_opportunity_v33_retry_20260804_134141`
- X-Plane prefix:
  `logs/xplane/xplane_visual_opportunity_v33_retry_20260804_134141`
- gaze:
  `logs/tobii/session_xplane_tobii_20260804_134103_gaze.csv`
- STATE/INTRUDER rows: `3083 / 3083`
- EVENT rows: `101`
- gaze rows/valid: `21199 / 17970`, overall validity `84.8%`
- trials: `6/6 clean`, RIGHT/FRONT/LEFT 각 2회
- spawn/opportunity/baseline/response-start/response-end/min-distance/trial-end:
  각 trial당 정확히 1회
- opportunity geometry:
  RIGHT `x=3877.9~3878.9 px`, FRONT `x=2880.0 px`,
  LEFT `x=1882.3~1882.4 px`; relative bearing는 각각 `+32/0/-32°`
- `lights_write_ok=true`, `recognition_claim=false`: `6/6`
- initial span: RIGHT `12.5~13.8 px`, FRONT `8.8~8.9 px`,
  LEFT `13.8~14.4 px`
- Java/Tobii strict clean rate: `100%`
- trial gaze validity: `82.5~91.2%`
- primary opportunity-to-response: `12.323, 10.337, 6.166, 4.571,
  1.636, 0.849 s`; mean `5.983 s`, median `5.369 s`
- response axis: pitch 2회, throttle 4회
- hazard entered: `3/6`; advisory event는 설계대로 `0`
- 20 px 전에 response가 기록된 trial: `4/6`; 20 px event는 계속 보조
  진단값으로 취급한다.

Interpretation limits:

- 이 retry의 무작위 순서는 `RIGHT, RIGHT, FRONT, FRONT, LEFT, LEFT`였고 spawn
  AGL은 `2032, 1837, 1446, 1072, 814, 526 ft`로 계속 낮아졌다.
- opportunity-to-response와 trial order의 Pearson correlation은 `r=-0.986`이다.
  방향, trial order, 접근 고도, 반복학습이 완전히 분리되지 않으므로 이 한
  세션의 RIGHT/FRONT/LEFT 평균을 방향 효과로 해석하지 않는다.
- `clean`은 recording completeness를 뜻한다. trial 6에서는 response
  `0.849 s`가 first operational-external `1.692 s`보다 먼저였고 throttle이
  trigger였으므로 speed-task input 또는 ambiguous response일 수 있다.
- 본실험 분석에는 `recording_complete`와 별도의 behavior/avoidance validity,
  active-at-opportunity, gaze timing, trajectory improvement 기준이 필요하다.
- v33의 physical screen/light/event pipeline은 검증 완료됐다. 다음 분석
  보완 우선순위는 회피반응과 속도과제 조작의 분리 및 dynamic intruder/runway
  AOI다.

## 2026-08-05 v35 bounded speed-stability spawn gate

- 활성 Lua/version/protocol은 `FLYWITHLUA_STUDY_INTEGRATED.lua`,
  `260805_speed_stability_spawn_gate_v35`, `audio_task_v35`다.
- v34의 두 균형 방향 블록, 자동 6-trial, 10~15초 washout·자세 안정 판정,
  v33의 ±32°/0° intruder 형상과 visual-opportunity anchor를 유지한다.
- 음성 지시 시작 시각을 기준으로 candidate spawn 시각을 `6~10 s`에서
  무작위 선정한다.
- candidate 시각 이후 목표 속도 `±5 KIAS`, IAS 변화율 `±1 KIAS/s`를
  `1.5 s` 연속 만족하면 intruder를 생성한다. 변화율 표본 간격은 최소
  `0.25 s`다.
- 속도 안정조건을 만족하지 못해도 음성 시작 `14 s`에는 강제 생성한다.
  따라서 trial이 속도과제 미완료로 무기한 정지하지 않는다.
- 참가자 화면에는 gate 시각이나 안정 여부를 추가로 표시하지 않는다.
  operator overlay에서만 candidate/timeout/error/rate/stable 진단값을 본다.
- `SCENARIO_SELECTED`는 gate method, candidate/timeout과 허용기준을 기록한다.
  `INTRUDER_SPAWNED`는 실제 audio-to-spawn, 안정 여부, 속도오차·변화율,
  연속 안정시간, candidate 이후 대기, timeout 강제 여부를 기록한다.
- Java session/batch와 Tobii 병합 분석은 새 metadata를 export하며, 안정 생성은
  수치 기준까지 맞는지 확인하고 제한시간 생성은 실제로 14초 부근인지
  검증한다. metadata 누락은 정상값으로 추정하지 않고 timing invalid로 둔다.
- v33 retry 6-trial STATE 로그의 읽기 전용 근사 재생에서 `±3 KIAS`는
  `3/6`, `±5 KIAS`는 `1/6`이 14초 제한시간 생성으로 예상됐다. 후자는 기존
  `within_5_kias` 분석 기준과 일치하고 목표를 크게 놓친 trial만 제한시간에
  남겨 최종 기준으로 채택했다. 이는 v35 실제 runtime 검증을 대체하지 않는다.
- Java 21 compile, 자동 event smoke, v35 안정/강제-timeout 분석 smoke,
  Python analyzer/classifier smoke, batch 117-column export 일치 검증이 통과했다.
- 저장소 기준본, v35 스냅샷, 실제 X-Plane Scripts 실행본 SHA-256은
  `D9604131AF8A7EDB63EE906221127860287CCA458D09FE24027151421C4B132B`로 일치한다.
- Lua interpreter가 로컬 PATH에는 없지만 2026-08-05 X-Plane `Log.txt`에서
  `script loaded: 260805_speed_stability_spawn_gate_v35`를 확인했다. Lua 오류나
  quarantine 이동은 없어 reload 검증은 PASS다. 다음 6-trial validation에서
  안정 생성/강제 생성 비율과 event detail을 확인한 뒤 기준을 동결한다.

## 2026-08-05 v35 first physical validation result
...
- Tobii gaze:
  `logs/tobii/session_xplane_tobii_20260805_154831_gaze.csv`
- final rows: STATE `3049`, INTRUDER `3049`, EVENT `103`; `RECEIVER_STOP`과
  `SESSION_STOP`이 기록됐다.
- 6개 trial은 모두 recording clean이며 방향표는
  `FRONT|LEFT|RIGHT|LEFT|FRONT|RIGHT`로 두 블록 균형과 인접 비반복 조건을
  만족했다. 목표속도는 `95|90|105|110|95|100 KIAS`다.
- 모든 trial의 v35 audio/spawn timing metadata는 valid다.
- stable spawn은 trial `2,4,6`, 14초 forced timeout은 trial `1,3,5`로
  현재 gate의 stable rate는 `3/6`이다.
- trial 3은 spawn 시 목표보다 `6.499 KIAS` 낮고 IAS rate가
  `+1.412 KIAS/s`여서 실제 미완료로 해석한다.
- trial 1과 5는 spawn 시 오차가 각각 `+0.297/+0.299 KIAS`였지만 연속
  안정시간이 `0.110/0.838 s`에 불과해 timeout이 됐다. 이는 error band보다
  raw short-window IAS-rate 조건이 gate를 반복 reset한 결과다.
- 같은 pre-spawn STATE를 읽기 전용으로 재생했을 때 rate sample window를
  `0.25~1.0 s`로만 늘려도 forced `3/6`은 변하지 않았다. rate tolerance를
  `±1.25 KIAS/s`로 완화하면 forced `1/6`이지만, 단일 pilot 자료에 맞춘 새
  derivative cutoff가 될 위험이 있다.
- 더 단순한 후보는 IAS rate를 gate 필수조건에서 빼고 목표 `±5 KIAS`를
  `1.5 s` 연속 유지하도록 하는 것이다. 같은 자료의 후향 검사는 stable
  `5/6`, forced `1/6`이며 실제 목표 미완료 trial 3만 forced로 남았다.
  IAS rate는 삭제하지 않고 spawn diagnostic/covariate로 계속 기록한다.
- Tobii 전체 gaze는 `29030` rows, valid `21905 (75.5%)`; trial별 validity는
  `79.2~92.6%`다. 모든 trial에서 response 전 operational-external evidence가
  존재했다.
- behavior classifier 결과는 `AMBIGUOUS 4`, `AVOIDANCE_CANDIDATE 1`,
  `SPEED_TASK_CORRECTION 1`이다. stable gate만으로 speed-task와 avoidance
  intent가 완전히 분리되지는 않았다.
- 현재 v35는 기능 검증 PASS지만 본실험 기준 동결 상태는 아니다. 다음 권장
  변경은 candidate `6~10 s`, timeout `14 s`, `±5 KIAS/1.5 s`를 유지하면서
  IAS rate를 필수 gate가 아닌 기록 진단값으로 내리는 것이다. 변경 시 v36으로
  버전업하고 동일 6-trial validation을 한 번 더 수행한다.

## 2026-08-05 v36 target-band dwell spawn gate

- 활성 version/protocol/profile은 `260805_target_band_dwell_spawn_gate_v36`,
  `audio_task_v36`, `visual_opportunity_proxy_v36`이다.
- candidate `U(6,10 s)`, timeout `14 s`, 목표오차 `±5 KIAS`, dwell `1.5 s`는
  v35와 동일하다.
- 필수 생성조건은 `abs(current_ias - target_ias) <= 5 KIAS`가 `1.5 s` 연속
  유지되는 것뿐이다. v35의 `abs(ias_rate) <= 1 KIAS/s` 조건은 제거했다.
- IAS rate 계산 간격 `0.25 s`와 `speed_rate_at_spawn_kias_s`,
  `speed_rate_valid` 기록은 그대로 유지한다. event detail에는
  `speed_rate_diagnostic_only=true`를 명시한다.
- 새 gate method는 `bounded_speed_band_dwell_v2`, condition은
  `target_error_band_dwell`이다. v35의 `bounded_speed_stability_v1` 자료와
  분석에서 구별된다.
- rate condition 제거 외에는 자동 6-trial, 두 방향 균형 블록, washout/자세
  안정, intruder 형상·조명·opportunity, response detector를 바꾸지 않았다.
- Java/Python timing validation은 v36 stable spawn에서 error band와 dwell만
  요구한다. rate가 크거나 invalid여도 diagnostic으로 보존하며 timing-valid를
  거부하지 않는다. v35 자료는 종전 rate 필수 규칙으로 계속 판정한다.
- 합성 v36 trial에서 rate `-2.0 KIAS/s`, rate-valid false여도 error/dwell이
  맞으면 timing-valid가 true이고, 같은 rate의 v35 trial은 false임을 확인했다.
- Java 21 compile, auto-event smoke, random-speed/v36 analysis smoke, Python
  analyzer/classifier smoke, v36 batch `117/117` columns가 통과했다.
- 실제 v35 session을 새 Java/Python 코드로 재분석해 Java `6/6 clean`, v35
  timing-valid `6/6`, gaze `29030/21905 valid`가 유지됨을 확인했다.
- 저장소 기준본, v36 스냅샷, 실제 X-Plane Scripts 실행본 SHA-256은 모두
  `1D015BF392ADF09ECDFB4AA32275B804A3F0D76AC58A8C50D7829C2B850AA3BA`로
  일치한다. 실제 Scripts 폴더에는 활성 통합 Lua 한 개만 있다.
- 로컬 PATH에 lua/luac는 없지만 실제 FlyWithLua reload에서
  `script loaded: 260805_target_band_dwell_spawn_gate_v36`와
  `All script files loaded successfully`를 확인했다. quarantine Lua는 0개다.
- 동일 조건 6-trial 측정도 완료했으며 결과는 아래 first physical validation
  section을 따른다.

## 2026-08-05 v36 first physical validation result

- session/prefix:
  `session_speed_gate_v36_validation_20260805_162420` /
  `logs/xplane/xplane_speed_gate_v36_validation_20260805_162420`
- Tobii gaze:
  `logs/tobii/session_xplane_tobii_20260805_162344_gaze.csv`
- final rows는 STATE `2841`, INTRUDER `2841`, EVENT `105`, gaze `25185`이며
  `RECEIVER_STOP`과 `SESSION_STOP`이 기록됐다.
- 6개 trial 모두 reset/start/spawn/visual-opportunity/detectability/response/end가
  있고 Java strict clean `6/6`, v36 audio/spawn timing-valid `6/6`이다.
- 방향표는 `LEFT|FRONT|RIGHT|LEFT|FRONT|RIGHT`이고 두 3-trial block 각각
  LEFT/FRONT/RIGHT를 한 번씩 포함한다. 속도표는
  `105|95|100|90|90|110 KIAS`다.
- target-band stable spawn은 trial `1,2,3,5`의 `4/6`, 14초 forced spawn은
  trial `4,6`의 `2/6`이다. forced trial의 spawn error는 각각
  `+6.893/-7.835 KIAS`로 실제 `±5 KIAS` 미달성이다.
- stable trial 2와 3의 spawn rate는 각각 `-1.020/+1.199 KIAS/s`였지만 v36은
  이를 diagnostic으로만 보존해 생성 조건을 막지 않았다. 목표오차가 작은데
  raw rate 때문에 forced가 된 v35 trial 1/5 유형은 이번 v36 결과에 없다.
- raw gaze validity는 `20748/25185 = 82.4%`, trial별 validity는
  `84.6~92.6%`, trial 동일가중 평균은 `89.2%`다. measured side episode는
  `58`, inferred episode는 `25`다.
- visual-opportunity-to-response는
  `1.956, 5.451, 5.717, 3.394, 6.461, 3.435 s`, 평균 `4.402 s`다.
  모든 trial에서 response 전에 operational-external evidence가 있지만 이는
  intruder acquisition의 확정값이 아니다.
- 6개 trial 모두 초기 estimated span `8.556~14.259 px`가 5 px 기준보다 커서
  `INTRUDER_VISUAL_OPPORTUNITY_ONSET`이 spawn과 동시에 기록됐다. 이번 session의
  primary latency는 수치상 spawn-to-response와 같다. 20 px detectability는
  spawn 후 `5.160~11.283 s`에 발생했고 trial 3만 그 뒤 `0.557 s`에 response가
  기록됐으며 나머지 5개는 20 px 이전 response다.
- behavior evidence는 `AMBIGUOUS 4`, `AVOIDANCE_CANDIDATE 2`다. trial 4는
  speed와 avoidance evidence가 동시에 있어 ambiguous이며, 이 분류도 pilot
  intent 확정값은 아니다.
- v36 gate의 engineering validation은 PASS다. 본실험 분석에서는 recording
  clean과 stable/forced gate status를 분리하고, forced trial을 별도 층화 또는
  sensitivity 대상으로 유지한다.

## 2026-08-06 group-meeting feedback and gaze-grid direction

- 교수 피드백에 따라 speed correction인지 intruder avoidance인지 조종 의도를
  반드시 분류하는 것은 현재 연구의 필수 목표에서 제외한다.
- `classify_xplane_tobii_behavior.py`와 기존 behavior output은 삭제하지 않지만
  primary outcome이 아니라 보조 품질진단/sensitivity 도구로 유지한다.
- 다음 핵심 측정은 화면상 intruder 위치·추정 크기로 객관적 visual opportunity를
  만들고, 그 이후 response latency를 계산하는 것이다. 실제 인지시점이라고
  단정하지 않는다.
- 교수의 grid 제안은 화면을 고정 spatial cell로 나눠 gaze distribution과
  transition을 기록하는 방법론으로 해석한다. grid 자체는 gaze 목적 범주를
  결정하지 않는다.
- semantic external class 후보는 `RUNWAY_GUIDANCE`,
  `INTRUDER_SEARCH_OR_TRACKING`, `OTHER_EXTERNAL`, `UNRESOLVED`다.
- 좌우 monitor의 방향이 실제 intruder 생성 방향과 일치하는 measured side gaze는
  `INTRUDER_SEARCH_PROXY`로 볼 수 있다. 반대 방향, FRONT trial의 side gaze,
  방향 불명 inferred loss는 확정 intruder gaze로 처리하지 않는다.
- 정면 monitor는 fixed grid를 보조 spatial index로 사용하되, 움직이는 intruder는
  projected screen x/y/span 기반 dynamic AOI, runway는 별도 runway AOI로
  판정한다. grid cell만으로 작은 moving intruder를 확정하지 않는다.
- 현재 v36은 spawn 시 beacon/nav/strobe를 실제로 ON으로 설정한다. 다음 scenario
  revision에서는 artificial saliency를 제거하고 geometry-based opportunity로
  전환해야 한다.
- 다음 intruder geometry는 고정 생성거리보다 방향별 initial projected span과
  spawn-to-opportunity 목표시간을 먼저 정하고, session 시간을 유지하는 범위에서
  거리와 속도를 역산하는 방식으로 설계한다. 세부 span/시간/속도는 짧은
  FRONT/LEFT/RIGHT physical calibration 전에는 동결하지 않는다.

## 2026-08-11 v37 geometry-opportunity calibration rule

- 활성 Lua/version/protocol/profile은 `FLYWITHLUA_STUDY_INTEGRATED.lua`,
  `260811_geometry_opportunity_calibration_v37`, `audio_task_v37`,
  `visual_opportunity_proxy_v37`이다.
- 보관 스냅샷은
  `FLYWITHLUA_STUDY_INTEGRATED_260811_geometry_opportunity_calibration_v37.lua`다.
- crossing 및 수동 head-on intruder의 beacon/nav/strobe/landing/taxi light를
  모두 OFF로 설정한다. `lights_write_ok=true`는 OFF 값 쓰기에 성공했다는 뜻이며
  조명이 켜졌다는 뜻이 아니다.
- `INTRUDER_VISUAL_OPPORTUNITY_ONSET`은 spawn 시 5 px와 조명을 결합하던 v36
  방식 대신, projected intruder span이 최초 `20 px`에 도달한 시점에 기록한다.
- v37 opportunity method는 `geometry_span_operational_proxy_v2`이고
  `recognition_claim=false`, `light_assisted=false`, `lights=off`를 기록한다.
  이 event는 표준화된 관측기회 proxy이며 참가자의 실제 발견·인지시점을
  직접 측정한 값으로 표현하지 않는다.
- 기존 `INTRUDER_VISUALLY_DETECTABLE` 20 px event도 후방 호환과 진단을 위해
  유지한다. 정상 순서는 같은 update에서 opportunity가 먼저, detectability가
  다음에 기록되는 순서다.
- crossing base speed는 `58 -> 65 m/s`로 조정해 conflict time을 유지하면서
  초기 생성거리를 늘린다. 최종 파라미터 동결 전 교정 목표는 initial span
  `7~13 px`, spawn-to-opportunity `5~11 s`이며, 현재는 합격을 강제하지 않고
  spawn/opportunity event metadata로 측정한다.
- Java `XPlaneSessionAnalysisMain`과 Python
  `analyze_xplane_tobii_session.py`는 v37을 v36과 같은 target-band dwell timing
  규칙 및 visual-opportunity clean protocol로 처리한다. v35/v36 후방 호환을
  유지한다.
- 검증은 Java 21 compile, auto-event smoke, v37 analysis smoke, Python compile,
  analyzer/classifier smoke, 실제 v36 6-trial Java/Python 회귀가 모두 PASS했다.
- 저장소 기준본·v37 snapshot·실제 X-Plane Scripts 실행본 SHA-256은
  `002B1235B3C2BE6A7833435D6CCF91F876874DF7A8A575D06794705D310328F7`로
  일치한다.
- 2026-08-11 실제 FlyWithLua reload에서
  `script loaded: 260811_geometry_opportunity_calibration_v37`와
  `All script files loaded successfully`를 확인했다. 활성 Scripts Lua는 1개,
  `Scripts (Quarantine)` Lua는 0개다. 다음 단계는 Java/Tobii를 실행한 뒤
  FRONT/LEFT/RIGHT physical calibration을 수집하는 것이다.

## 2026-08-11 v37 first physical calibration result

- session/prefix는 `session_geometry_calibration_v37_20260811_144334` /
  `logs/xplane/xplane_geometry_calibration_v37_20260811_144334`이고 gaze는
  `logs/tobii/session_xplane_tobii_20260811_144244_gaze.csv`다.
- STATE/INTRUDER/EVENT/gaze rows는 `3209/3209/101/34432`이며 Java와 Tobii를
  정상 종료했다. Java strict clean은 `6/6`이다.
- 방향은 `RIGHT|FRONT|LEFT|RIGHT|FRONT|LEFT`로 각 방향 2회다.
- 모든 spawn은 `profile=visual_opportunity_proxy_v37`, `lights=off`,
  `lights_write_ok=true`이고 모든 opportunity는
  `method=geometry_span_operational_proxy_v2`, `recognition_claim=false`다.
  이는 OFF dataref write 성공이며 실제 화면 점멸 소멸은 참가자의 육안 확인을
  함께 남겨야 한다.
- trial별 initial span은 `13.384, 8.021, 12.087, 13.191, 8.547, 13.233 px`,
  spawn-to-opportunity는 `6.124, 10.949, 7.979, 6.308, 10.131, 6.519 s`다.
- delay 목표 `5~11 s`는 `6/6` 통과했다. initial span의 잠정 `7~13 px`는
  `3/6`이지만 초과 3건은 `13.191~13.384 px`의 작은 경계 초과다. 실제 전체
  범위는 `8.021~13.384 px`다.
- 방향 평균 initial span/delay는 FRONT `8.284 px / 10.540 s`, LEFT
  `12.660 px / 7.249 s`, RIGHT `13.288 px / 6.216 s`다. 20 px opportunity
  range 평균은 FRONT `884.2 m`, LEFT `1221.6 m`, RIGHT `1227.6 m`다.
- opportunity-to-response는 평균 `1.800 s`, 중앙값 `1.936 s`, 범위
  `0.347~3.186 s`이고 모두 opportunity 이후다. trial 2는
  `active_control_at_visual_opportunity=true`라 매우 짧은 `0.347 s`를 순수한
  새 회피반응으로 단정하지 않는다.
- 전체 raw gaze validity는 `69.8%`지만 session 전후 비행/대기구간을 포함한다.
  trial별 validity는 `85.4~92.7%`, 동일가중 평균 약 `88.1%`로 분석 가능하다.
- geometry calibration과 별개로 speed spawn gate는 stable `0/6`, forced timeout
  `6/6`이다. spawn error는 `+5.967,-11.999,+6.418,-5.764,-5.594,+6.295 KIAS`다.
  recording clean과 timing-valid가 모두 true여도 속도과제 달성을 의미하지 않는다.
- 판정은 geometry candidate PASS, speed-task scheduling FAIL이다. 다음 revision은
  20 px/65 m/s geometry를 우선 유지하고, 독립적인 절대속도 무작위 선택보다
  audio 시점 IAS 근처의 실현 가능한 5-knot 단위 목표를 선택하는 방식을 검토한다.

## 2026-08-28 technical pilot dataset and publication rule

- 2026-08-25~26 연구실 기술 파일럿은 연구실 참가자 7명과 experienced operator
  reference 1명으로 수행했다. 보고서와 논문에서는 실명 대신 `P01~P07`과
  `EXPERIENCED_REFERENCE`를 사용한다. 원본 gaze/X-Plane CSV는 Git에 올리지 않는다.
- primary lab set은 자동 6-trial을 완주한 6명, 총 36 trial이다. LEFT/RIGHT/FRONT는
  각 12 trial이다. 부분 세션 1명은 수면 충돌 전 4 trial만 보존하며 primary 평균에
  넣지 않고 sensitivity로만 사용한다.
- complete-six trial 동일가중 gaze validity는 약 `96.3%`, response 검출은
  `31/36`, visual-opportunity-to-response 검출값 평균/중앙값은 약
  `3.135/2.138 s`다. opportunity 당시 control active trial은 별도 flag와 sensitivity를
  유지하며 실제 인지시간이나 순수한 새 회피반응으로 부르지 않는다.
- complete-six `FULL_TRIAL` DTMC는 eligible bins `6077`, transitions `5942`,
  self `4745 (79.9%)`, switches `1197`이다. participant-equal 평균 전환빈도는
  약 `59.0/min`이다. 참가자별 operational-external은 `4.0~71.7%`로 개인차가 크다.
- complete-six speed gate는 stable `16/36`, forced timeout `20/36`이다. 특히
  target 110 KIAS는 `7/7` forced였다. 이 자료로 speed target이나 direction의
  인과효과를 주장하지 않는다.
- 최신 experienced reference는
  `session_main_KIM_SEOKHO_20260826_164645`이며 6/6 clean이다. trial gaze validity
  `91.5%`, operational-external `48.4%`, response 전체 평균 `2.467 s`, opportunity
  당시 active-control 제외 평균 `3.834 s`, DTMC self `74.0%`, switches
  `74.4/min`, 최소 수평거리 평균 `35.0 m`다. 이 세션을 reference 본문값으로 사용하고
  2026-08-12 self session은 반복 재현성 sensitivity로만 보존한다.
- APISAT 원고는 조종사 우열 비교가 아니라 event-aligned X-Plane/Tobii data pipeline,
  dynamic AOI, phase-specific DTMC/next-switch의 technical pilot validation으로 쓴다.
- KAI 항공우주논문상 후보 분야는 `미래SW`다. 마린온/MUH 시뮬레이터 적용은 실제
  검증 완료가 아니라 simulator adapter와 교관 디브리핑 소프트웨어의 후속 이식
  가능성으로 제한한다. APISAT과 KAI 원고는 중복게재 규정을 별도로 확인한다.
- 사람 대상 결과의 학술발표 사용은 IRB 승인 또는 기관의 공식 면제 판단을 먼저
  확인한다. 현재 기술 파일럿을 허가 없이 본실험 참가자 자료로 일반화하지 않는다.

## 2026-08-28 APISAT manuscript generation rule

- 기준 full-paper 초안은 `2026 APISAT/APISAT-2026_FullPaper_Draft_260828.md`이고,
  `tools/docs/create_apisat_fullpaper_draft.ps1`로 DOCX/PDF를 재생성한다.
- 생성본은 APISAT A4 여백, 2단 본문, 100-200 word abstract, 10-page 이내 규격을 유지한다.
- figure 문단은 정확한 줄높이를 사용하면 PDF에서 잘리므로 single/automatic line spacing을 유지한다.
...
## 2026-09-08 문서 및 Git 공개 범위 정리

- 2026-09-01까지의 APISAT full-paper 초안 생성 작업과 해병항공 전투발전 워크숍 발표자료 작업을 기준 문서에 반영했다.
- 워크숍 PPTX/PDF, 발표 개요 `README.md`, 재생성 스크립트와 필요한 AOI/DTMC 그림을 Git 추적 대상으로 묶었다.
- 발표자료 생성 스크립트는 Git에서 제외된 `build_atc_tmp`와 `2026 APISAT` 내부 그림을 직접 참조하지 않고, 워크숍 폴더의 `assets`만 참조하도록 수정했다.
- `2026 APISAT/`는 공식 양식, 참고 자료와 로컬 논문 산출물이 섞인 연구자료 폴더이므로 기존 `.gitignore` 정책을 유지한다.
- 원본 실험 CSV와 `logs/`, 임시 빌드·분석 산출물은 계속 Git에 포함하지 않는다.
- `.idea/misc.xml`의 개인 IntelliJ 설정 변경은 이번 연구 산출물 커밋에서 제외한다.
