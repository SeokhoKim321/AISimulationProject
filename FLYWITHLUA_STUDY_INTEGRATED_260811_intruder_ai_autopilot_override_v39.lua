...
local STUDY_SCRIPT_VERSION = "260811_intruder_ai_autopilot_override_v39"
local PARTICIPANT_NOTICE_DURATION_S = 2.5
local TASK_AUDIO_PROTOCOL = "audio_task_v39"
local APPROACH_SCHEDULE_METHOD = "two_balanced_random_blocks_v1"
local TASK_AUDIO_BASELINE_DELAY_S = 3.0
local SPEED_GATE_CANDIDATE_MIN_S = 6.0
local SPEED_GATE_CANDIDATE_MAX_S = 10.0
local SPEED_GATE_TIMEOUT_S = 14.0
local SPEED_GATE_ERROR_TOLERANCE_KIAS = 5.0
local SPEED_GATE_STABILITY_DURATION_S = 1.5
local SPEED_RATE_DIAGNOSTIC_SAMPLE_INTERVAL_S = 0.25
...
local INTRUDER_VISUAL_OPPORTUNITY_THRESHOLD_PX = 20.0
local CROSSING_CALIBRATION_INITIAL_SPAN_MIN_PX = 7.0
local CROSSING_CALIBRATION_INITIAL_SPAN_MAX_PX = 13.0
local CROSSING_CALIBRATION_OPPORTUNITY_DELAY_MIN_S = 5.0
local CROSSING_CALIBRATION_OPPORTUNITY_DELAY_MAX_S = 11.0
local CROSSING_SIDE_INITIAL_BEARING_DEG = 32.0
local CROSSING_FRONT_INITIAL_BEARING_DEG = 0.0
...
crossing_delay_min_s = SPEED_GATE_CANDIDATE_MIN_S
crossing_delay_max_s = SPEED_GATE_CANDIDATE_MAX_S
crossing_planned_audio_to_spawn_s = 0.0
crossing_spawn_timeout_time_s = 0.0
crossing_speed_previous_kias = 0.0
crossing_speed_previous_time_s = -1.0
crossing_speed_rate_kias_s = 0.0
crossing_speed_rate_valid = false
crossing_speed_error_kias = 0.0
crossing_speed_stable_since_s = -1.0
crossing_speed_stability_duration_s = 0.0
crossing_speed_stable = false
crossing_speed_gate_pass_time_s = -1.0
crossing_speed_stable_before_spawn = false
crossing_spawn_forced_by_timeout = false
...
crossing_speed_mps = 65.0
crossing_effective_speed_mps = 65.0
crossing_spawn_extra_distance_m = 0.0
...
crossing_visual_opportunity_recorded = false
crossing_desired_initial_bearing_deg = 0.0
crossing_initial_range_m = 0.0
crossing_lights_write_ok = false
intruder_lights_force_initialized = false
intruder_lights_write_ok = false
intruder_lights_readback_ok = false
intruder_lights_all_off = false
intruder_lights_reenabled_count = 0
intruder_lights_reenabled_event_recorded = false
intruder_light_beacon_value = -1.0
intruder_light_nav_value = -1.0
intruder_light_strobe_value = -1.0
intruder_light_landing_value = -1.0
intruder_light_taxi_value = -1.0
intruder_ai_autopilot_override_write_ok = false
intruder_ai_autopilot_override_readback_ok = false
intruder_ai_autopilot_override_value = -1.0
...
function study_record_geometry_visual_opportunity(projection)
    if crossing_visual_opportunity_recorded
        or not projection.valid
        or not projection.visible
        or projection.span_px < INTRUDER_VISUAL_OPPORTUNITY_THRESHOLD_PX then
        return false
    end

    crossing_visual_opportunity_recorded = true
    local spawn_to_opportunity_s = math.max(
        0.0,
        running_time_sec - crossing_stabilization_start_time_s
    )
    study_record_event(
        "INTRUDER_VISUAL_OPPORTUNITY_ONSET",
        study_trial_detail(
            string.format(
                "method=geometry_span_operational_proxy_v2;operational_proxy=true;recognition_claim=false;opportunity_basis=projected_span_threshold;light_assisted=false;lights=off;lights_persistent_force=true;lights_write_ok=%s;threshold_px=%.3f;minimum_span_px=%.3f;estimated_span_px=%.3f;screen_x_px=%.3f;screen_y_px=%.3f;slant_distance_m=%.3f;horizontal_distance_m=%.3f;relative_bearing_deg=%.3f;desired_relative_bearing_deg=%.3f;relative_elevation_deg=%.3f;display_width_px=%.0f;display_height_px=%.0f;horizontal_fov_deg=%.3f;spawn_to_opportunity_s=%.3f;calibration_delay_target_min_s=%.3f;calibration_delay_target_max_s=%.3f",
                tostring(crossing_lights_write_ok),
                INTRUDER_VISUAL_OPPORTUNITY_THRESHOLD_PX,
                INTRUDER_VISUAL_OPPORTUNITY_THRESHOLD_PX,
                projection.span_px,
                ...
                crossing_desired_initial_bearing_deg,
                projection.relative_elevation_deg,
                STUDY_DISPLAY_WIDTH_PX,
                STUDY_DISPLAY_HEIGHT_PX,
                STUDY_HORIZONTAL_FOV_DEG,
                spawn_to_opportunity_s,
                CROSSING_CALIBRATION_OPPORTUNITY_DELAY_MIN_S,
                CROSSING_CALIBRATION_OPPORTUNITY_DELAY_MAX_S
            )
            .. ";"
            .. study_ownship_context_detail()
            .. ";"
            .. study_intruder_lights_status_detail()
        )
    )
    return true
end
...
function study_update_crossing_visual_detectability(intruder_x, intruder_y, intruder_z)
    ...
    study_record_geometry_visual_opportunity(projection)
...
function study_build_balanced_approach_schedule()
    local first_block = {"left", "right", "front"}
    study_shuffle_table(first_block)

    local second_block = nil
    repeat
        second_block = {"left", "right", "front"}
        study_shuffle_table(second_block)
    until second_block[1] ~= first_block[3]

    return {
        first_block[1], first_block[2], first_block[3],
        second_block[1], second_block[2], second_block[3]
    }
end

function study_current_approach_block_index()
    if not study_auto_session_active or study_auto_session_trial_index < 1 then
        return 0
    end
    return math.floor((study_auto_session_trial_index - 1) / 3) + 1
end
...
function study_reset_crossing_speed_gate()
    crossing_spawn_timeout_time_s = 0.0
    crossing_speed_previous_kias = 0.0
    crossing_speed_previous_time_s = -1.0
    crossing_speed_rate_kias_s = 0.0
...
function study_update_crossing_speed_gate()
    if not random_crossing_armed or task_audio_current_option == nil then
        return false
    end

    local current_speed_kias = ias_mps
    crossing_speed_error_kias = current_speed_kias - task_audio_current_option.target_speed_kias

    if crossing_speed_previous_time_s < 0.0 then
        crossing_speed_previous_kias = current_speed_kias
        crossing_speed_previous_time_s = running_time_sec
    end

    local rate_sample_duration_s = running_time_sec - crossing_speed_previous_time_s
    if rate_sample_duration_s >= SPEED_RATE_DIAGNOSTIC_SAMPLE_INTERVAL_S then
        crossing_speed_rate_kias_s = (
            current_speed_kias - crossing_speed_previous_kias
        ) / rate_sample_duration_s
        crossing_speed_previous_kias = current_speed_kias
        crossing_speed_previous_time_s = running_time_sec
        crossing_speed_rate_valid = true
    end

    local within_gate = math.abs(crossing_speed_error_kias) <= SPEED_GATE_ERROR_TOLERANCE_KIAS

    if not within_gate then
        crossing_speed_stable_since_s = -1.0
        crossing_speed_stability_duration_s = 0.0
        crossing_speed_stable = false
        crossing_speed_gate_pass_time_s = -1.0
    ...
    if crossing_speed_stable_since_s < 0.0 then
        crossing_speed_stable_since_s = running_time_sec
    end
    crossing_speed_stability_duration_s = running_time_sec - crossing_speed_stable_since_s
    crossing_speed_stable = crossing_speed_stability_duration_s >= SPEED_GATE_STABILITY_DURATION_S
    if crossing_speed_stable and crossing_speed_gate_pass_time_s < 0.0 then
        crossing_speed_gate_pass_time_s = running_time_sec
    end
    return crossing_speed_stable
end
...
function study_reset_trial(automatic)
    ...
    crossing_trigger_time_s = 0.0
    crossing_planned_audio_to_spawn_s = 0.0
    study_reset_crossing_speed_gate()
    crossing_min_distance_reported = false
    crossing_time_to_conflict_s = 10.0
    crossing_min_horizontal_distance_m = 999999.0
    crossing_start_distance_m = 700.0
    ...
    crossing_initial_range_m = 0.0
    crossing_lights_write_ok = false
    study_reset_intruder_light_monitor()
    visual_advisory_active = false
    frame_count = 0
    sample_index = 0
    study_event_counter = 0
...
function study_start_trial(automatic)
    ...
    study_record_event(
        ...
        study_trial_detail(
            ...
            .. ";approach_schedule_method="
            .. (study_auto_session_active and APPROACH_SCHEDULE_METHOD or "manual_random")
            .. ";approach_block_index=" .. tostring(study_current_approach_block_index())
        )
    )
...
function study_prepare_auto_session_schedules()
    study_auto_session_approach_schedule = study_build_balanced_approach_schedule()
...
function study_begin_next_auto_trial()
    ...
    study_record_event(
        ...
        study_trial_detail(
            ...
            .. ";approach_schedule_method=" .. APPROACH_SCHEDULE_METHOD
            .. ";approach_block_index=" .. tostring(study_current_approach_block_index())
...
function study_start_auto_session()
    ...
    study_record_event(
        ...
        study_trial_detail(
            ...
            .. ";approach_schedule_method=" .. APPROACH_SCHEDULE_METHOD
            .. ";approach_block_1=" .. study_schedule_text({
                study_auto_session_approach_schedule[1],
                study_auto_session_approach_schedule[2],
                study_auto_session_approach_schedule[3]
            })
            .. ";approach_block_2=" .. study_schedule_text({
                study_auto_session_approach_schedule[4],
                study_auto_session_approach_schedule[5],
                study_auto_session_approach_schedule[6]
            })
...
function study_set_intruder_lights(enabled)
    local light_value = 0
    if enabled then
        light_value = 1
    end
    ...
    local ok = true
    ok = study_try_set_dataref("sim/multiplayer/position/plane1_beacon_lights_on", light_value) and ok
    ok = study_try_set_dataref("sim/multiplayer/position/plane1_nav_lights_on", light_value) and ok
    ok = study_try_set_dataref("sim/multiplayer/position/plane1_strobe_lights_on", light_value) and ok
    ok = study_try_set_dataref("sim/multiplayer/position/plane1_landing_lights_on", 0) and ok
    ok = study_try_set_dataref("sim/multiplayer/position/plane1_taxi_light_on", 0) and ok
    return ok
end

function study_read_intruder_light_value(path)
...
function study_read_intruder_lights()
    local beacon_ok, beacon_value = study_read_intruder_light_value(
        "sim/multiplayer/position/plane1_beacon_lights_on"
    )
    local nav_ok, nav_value = study_read_intruder_light_value(
        "sim/multiplayer/position/plane1_nav_lights_on"
    )
    local strobe_ok, strobe_value = study_read_intruder_light_value(
        "sim/multiplayer/position/plane1_strobe_lights_on"
    )
    local landing_ok, landing_value = study_read_intruder_light_value(
        "sim/multiplayer/position/plane1_landing_lights_on"
    )
    local taxi_ok, taxi_value = study_read_intruder_light_value(
        "sim/multiplayer/position/plane1_taxi_light_on"
    )
    local readback_ok = beacon_ok and nav_ok and strobe_ok and landing_ok and taxi_ok
    local all_off = readback_ok
        and math.abs(beacon_value) < 0.5
        and math.abs(nav_value) < 0.5
        and math.abs(strobe_value) < 0.5
        and math.abs(landing_value) < 0.5
        and math.abs(taxi_value) < 0.5
    return readback_ok, all_off, beacon_value, nav_value, strobe_value, landing_value, taxi_value
end

function study_reset_intruder_light_monitor()
    intruder_lights_force_initialized = false
    intruder_lights_write_ok = false
    intruder_lights_readback_ok = false
    intruder_lights_all_off = false
...
function study_set_intruder_ai_autopilot_override(enabled)
    local requested_value = 0
    if enabled then
        requested_value = 1
    end
    ...
    local readback_ok, readback_value = pcall(function()
        return get("sim/operation/override/override_plane_ai_autopilot", 1)
    end)
    if not readback_ok or type(readback_value) ~= "number" then
        readback_value = -1.0
        readback_ok = false
    end

    intruder_ai_autopilot_override_write_ok = write_ok
    intruder_ai_autopilot_override_readback_ok = readback_ok
    intruder_ai_autopilot_override_value = readback_value
    return write_ok
        and readback_ok
        and math.abs(readback_value - requested_value) < 0.5
end

function study_update_intruder_light_readback()
    local readback_ok, all_off, beacon_value, nav_value, strobe_value, landing_value, taxi_value =
        study_read_intruder_lights()
    intruder_lights_readback_ok = readback_ok
    intruder_lights_all_off = all_off
    intruder_light_beacon_value = beacon_value
    intruder_light_nav_value = nav_value
    intruder_light_strobe_value = strobe_value
    intruder_light_landing_value = landing_value
    intruder_light_taxi_value = taxi_value
end

function study_intruder_lights_status_detail()
    return string.format(
        "lights_persistent_force=true;ai_autopilot_override_requested=true;ai_autopilot_override_write_ok=%s;ai_autopilot_override_readback_ok=%s;ai_autopilot_override_value=%.0f;lights_readback_ok=%s;lights_all_off=%s;lights_reenabled_count=%d;light_readback_beacon=%.0f;light_readback_nav=%.0f;light_readback_strobe=%.0f;light_readback_landing=%.0f;light_readback_taxi=%.0f",
        tostring(intruder_ai_autopilot_override_write_ok),
        tostring(intruder_ai_autopilot_override_readback_ok),
        intruder_ai_autopilot_override_value,
        tostring(intruder_lights_readback_ok),
        tostring(intruder_lights_all_off),
        intruder_lights_reenabled_count,
        intruder_light_beacon_value,
        intruder_light_nav_value,
        intruder_light_strobe_value,
        intruder_light_landing_value,
        intruder_light_taxi_value
    )
end

function study_initialize_intruder_lights_off()
    study_reset_intruder_light_monitor()
    study_set_intruder_ai_autopilot_override(true)
    intruder_lights_write_ok = study_set_intruder_lights(false)
    study_update_intruder_light_readback()
    intruder_lights_force_initialized = true
    return intruder_lights_write_ok
end

function study_enforce_intruder_lights_off()
    if not intruder_lights_force_initialized then
        return study_initialize_intruder_lights_off()
    end

    study_set_intruder_ai_autopilot_override(true)

    local pre_readback_ok, pre_all_off, pre_beacon, pre_nav, pre_strobe, pre_landing, pre_taxi =
        study_read_intruder_lights()
    if pre_readback_ok and not pre_all_off then
        intruder_lights_reenabled_count = intruder_lights_reenabled_count + 1
        if not intruder_lights_reenabled_event_recorded then
            intruder_lights_reenabled_event_recorded = true
            study_record_event(
                "SCENARIO_MARKER",
                study_trial_detail(
                    string.format(
                        "intruder_lights_reenabled_by_sim=true;pre_force_beacon=%.0f;pre_force_nav=%.0f;pre_force_strobe=%.0f;pre_force_landing=%.0f;pre_force_taxi=%.0f",
                        pre_beacon,
                        pre_nav,
                        pre_strobe,
                        pre_landing,
    ...
    local current_write_ok = study_set_intruder_lights(false)
    intruder_lights_write_ok = intruder_lights_write_ok and current_write_ok
    study_update_intruder_light_readback()
    return current_write_ok
end

function study_retire_intruder()
    study_set_intruder_lights(false)
    local state_write_ok = study_set_intruder_state(
        local_x,
        local_y - 3000.0,
        local_z,
        0.0,
    ...
    study_set_intruder_ai_autopilot_override(false)
    return state_write_ok
end
...
function study_start_intruder_headon()
    ...
    study_initialize_intruder_lights_off()
    study_record_event("SCENARIO_SELECTED", study_trial_detail("scenario=headon_intruder"))
    study_record_event(
        "INTRUDER_SPAWNED",
        study_trial_detail(
            "scenario=headon_intruder;lights=off;light_assisted=false;"
            .. study_intruder_lights_status_detail()
            .. ";"
            .. study_ownship_context_detail()
        )
...
function study_arm_random_crossing(audio_start_sim_time_s)
    ...
    crossing_spawn_timeout_time_s = audio_start_sim_time_s + SPEED_GATE_TIMEOUT_S
    random_crossing_armed = true
    intruder_crossing_active = false
    crossing_min_distance_reported = false

    study_status_text = string.format(
        "CROSSING SPEED GATE: %.1fs candidate / %.1fs timeout",
        crossing_planned_audio_to_spawn_s,
        SPEED_GATE_TIMEOUT_S
    )
    study_record_event(
        "SCENARIO_SELECTED",
        study_trial_detail(
            string.format(
                "scenario=random_final_approach_crossing;delay_anchor=task_audio_start;spawn_gate_method=bounded_speed_band_dwell_v2;speed_gate_condition=target_error_band_dwell;planned_audio_to_spawn_s=%.3f;candidate_audio_to_spawn_s=%.3f;spawn_timeout_audio_to_spawn_s=%.3f;speed_error_tolerance_kias=%.3f;speed_stability_required_s=%.3f;speed_rate_diagnostic_only=true;speed_rate_sample_interval_s=%.3f;audio_start_sim_time_s=%.3f;crossing_trigger_time_s=%.3f;crossing_timeout_time_s=%.3f",
                crossing_planned_audio_to_spawn_s,
                crossing_planned_audio_to_spawn_s,
                SPEED_GATE_TIMEOUT_S,
                SPEED_GATE_ERROR_TOLERANCE_KIAS,
                SPEED_GATE_STABILITY_DURATION_S,
                SPEED_RATE_DIAGNOSTIC_SAMPLE_INTERVAL_S,
                audio_start_sim_time_s,
                crossing_trigger_time_s,
                crossing_spawn_timeout_time_s
            )
        )
    )
end
...
function study_start_intruder_crossing()
    ...
    if scheduled_approach == "left" or (scheduled_approach == nil and approach_choice == 1) then
        intruder_side_sign = -1.0
        crossing_approach_mode = "left"
        crossing_desired_initial_bearing_deg = -CROSSING_SIDE_INITIAL_BEARING_DEG
    elseif scheduled_approach == "right" or (scheduled_approach == nil and approach_choice == 2) then
        ...
        crossing_desired_initial_bearing_deg = CROSSING_FRONT_INITIAL_BEARING_DEG
    end
    ...
    local desired_bearing_rad = math.rad(crossing_desired_initial_bearing_deg)
    local spawn_ray_x = intruder_forward_x * math.cos(desired_bearing_rad)
        + intruder_right_x * math.sin(desired_bearing_rad)
    local spawn_ray_z = intruder_forward_z * math.cos(desired_bearing_rad)
        + intruder_right_z * math.sin(desired_bearing_rad)
    local target_relative_x = crossing_target_x - local_x
    local target_relative_z = crossing_target_z - local_z
    local target_projection_m = target_relative_x * spawn_ray_x + target_relative_z * spawn_ray_z
    local target_distance_squared_m = target_relative_x * target_relative_x
        + target_relative_z * target_relative_z
    local target_perpendicular_squared_m = math.max(
        0.0,
        target_distance_squared_m - target_projection_m * target_projection_m
    )
    local path_discriminant_m = crossing_start_distance_m * crossing_start_distance_m
        - target_perpendicular_squared_m
    if path_discriminant_m < 1.0 then
        crossing_start_distance_m = math.sqrt(target_perpendicular_squared_m + 1.0)
        path_discriminant_m = 1.0
    end
    crossing_initial_range_m = target_projection_m + math.sqrt(path_discriminant_m)
    if crossing_initial_range_m < 1.0 then
        crossing_initial_range_m = 1.0
    end
    intruder_start_x = local_x + spawn_ray_x * crossing_initial_range_m
    intruder_start_z = local_z + spawn_ray_z * crossing_initial_range_m
    ...
    crossing_start_distance_m = to_target_len
    ...
    crossing_lights_write_ok = study_initialize_intruder_lights_off()
    ...
    local speed_gate_pass_audio_elapsed_s = -1.0
    if crossing_speed_gate_pass_time_s >= 0.0 then
        speed_gate_pass_audio_elapsed_s = crossing_speed_gate_pass_time_s - task_audio_start_sim_time_s
    end
    ...
    crossing_spawn_event_detail = study_trial_detail(
        string.format(
            "scenario=random_final_approach_crossing;profile=visual_opportunity_proxy_v39;geometry_calibration=true;spawn_phase=stabilization_start;delay_anchor=task_audio_start;spawn_gate_method=bounded_speed_band_dwell_v2;speed_gate_condition=target_error_band_dwell;speed_rate_diagnostic_only=true;planned_audio_to_spawn_s=%.3f;candidate_audio_to_spawn_s=%.3f;actual_audio_to_spawn_s=%.3f;spawn_timeout_audio_to_spawn_s=%.3f;speed_stable_before_spawn=%s;speed_error_at_spawn_kias=%.3f;speed_rate_at_spawn_kias_s=%.3f;speed_rate_valid=%s;speed_stability_duration_s=%.3f;speed_gate_pass_audio_elapsed_s=%.3f;spawn_wait_after_candidate_s=%.3f;spawn_forced_by_timeout=%s;approach=%s;side_sign=%.0f;desired_initial_bearing_deg=%.3f;planned_move_to_path_s=%.3f;ownship_arrival_to_path_s=%.3f;path_lead_time_s=%.3f;target_ahead_m=%.3f;initial_range_m=%.3f;path_distance_m=%.3f;spawn_extra_distance_m=%.3f;base_speed_mps=%.3f;effective_speed_mps=%.3f;vertical_speed_mps=%.3f;vertical_match_gain=%.3f;stabilization_s=%.3f;heading_deg=%.3f;display_width_px=%.0f;display_height_px=%.0f;horizontal_fov_deg=%.3f;opportunity_basis=projected_span_threshold;visual_opportunity_threshold_px=%.3f;detectability_threshold_px=%.3f;calibration_initial_span_target_min_px=%.3f;calibration_initial_span_target_max_px=%.3f;calibration_opportunity_delay_target_min_s=%.3f;calibration_opportunity_delay_target_max_s=%.3f;light_assisted=false;lights=off;lights_persistent_force=true;lights_write_ok=%s;initial_projection_valid=%s;initial_screen_visible=%s;initial_estimated_span_px=%.3f;initial_screen_x_px=%.3f;initial_screen_y_px=%.3f;initial_slant_distance_m=%.3f;initial_relative_bearing_deg=%.3f;initial_relative_elevation_deg=%.3f",
            crossing_planned_audio_to_spawn_s,
            crossing_planned_audio_to_spawn_s,
            actual_audio_to_spawn_s,
            SPEED_GATE_TIMEOUT_S,
            tostring(crossing_speed_stable_before_spawn),
            crossing_speed_error_kias,
            crossing_speed_rate_kias_s,
            tostring(crossing_speed_rate_valid),
            crossing_speed_stability_duration_s,
            speed_gate_pass_audio_elapsed_s,
            actual_audio_to_spawn_s - crossing_planned_audio_to_spawn_s,
            tostring(crossing_spawn_forced_by_timeout),
            ...
            INTRUDER_VISUAL_OPPORTUNITY_THRESHOLD_PX,
            INTRUDER_VISUAL_DETECTABILITY_THRESHOLD_PX,
            CROSSING_CALIBRATION_INITIAL_SPAN_MIN_PX,
            CROSSING_CALIBRATION_INITIAL_SPAN_MAX_PX,
            CROSSING_CALIBRATION_OPPORTUNITY_DELAY_MIN_S,
            CROSSING_CALIBRATION_OPPORTUNITY_DELAY_MAX_S,
            tostring(crossing_lights_write_ok),
    ...
    study_record_geometry_visual_opportunity(initial_projection)
    logMsg("[StudyIntegrated] crossing intruder spawned and stabilizing")
end

function study_update_random_crossing()
    if random_crossing_armed and trial_active then
        local speed_stable = study_update_crossing_speed_gate()
        local candidate_reached = running_time_sec >= crossing_trigger_time_s
        local timeout_reached = running_time_sec >= crossing_spawn_timeout_time_s
        if candidate_reached and speed_stable then
            crossing_speed_stable_before_spawn = true
            crossing_spawn_forced_by_timeout = false
            study_start_intruder_crossing()
        elseif timeout_reached then
            crossing_speed_stable_before_spawn = false
            crossing_spawn_forced_by_timeout = true
            study_start_intruder_crossing()
        end
    end
    ...
    if crossing_stabilizing then
        ...
        study_enforce_intruder_lights_off()
...
function study_draw_status()
    ...
    draw_string_Helvetica_12(
        ...
        536,
        string.format(
            "Visual span %.1f/%.1f px | Screen %.0f,%.0f | Visible %s | Gate %s | Opportunity %s",
            ...
            tostring(crossing_visual_opportunity_recorded)
        )
    )
    ...
    if random_crossing_armed then
        draw_string_Helvetica_18(
            ...
            474,
            string.format(
                "SPAWN GATE CAND %.1fs TIMEOUT %.1fs | ERR %.1fKT RATE %.2fKT/S | STABLE %s",
                math.max(0.0, crossing_trigger_time_s - running_time_sec),
                math.max(0.0, crossing_spawn_timeout_time_s - running_time_sec),
                crossing_speed_error_kias,
                crossing_speed_rate_kias_s,
                tostring(crossing_speed_stable)
            )
        )
    end
...
-- Release any stale slot-1 AI override left by an interrupted/reloaded trial,
-- then park the intruder outside the visible scene before callbacks begin.
study_retire_intruder()
