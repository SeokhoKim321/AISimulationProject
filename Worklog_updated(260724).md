...
APISAT 본실험 보완의 1단계로 final-approach 시나리오를 다시 정의했다. 최신 코드와 2026-07-20 same-PC X-Plane/Java/Tobii 세션을 대조한 결과, 기존 문서 상단이 v20·분리 PC·Tobii 미연동 상태에 머물러 있었고 최신 설계 논의도 반영되지 않은 것을 확인했다.

이번 작업의 핵심:

- `APISAT_EXPERIMENT_SCENARIO_BASELINE_260724.md` 작성
- intruder 직접 발견 실험의 기준점을 `ADVISORY_SHOWN`에서 `INTRUDER_SPAWNED`로 변경하는 계획 명문화
- participant 화면의 countdown/debug/advisory 숨김 계획 명문화
- 표준 녹음 음성, audio start 이후 독립적인 3~10초 spawn 계획 명문화
- 매 trial 동일 final-approach 저장상황 재설정 원칙 추가
- `recording/event complete`, `flight-condition-valid`, outcome 분리 원칙 추가
- 3모니터 Surround feasibility, dual Spark 제한, 객관적 detectability event와 dynamic AOI 후속계획 정리
- 기준 문서 상단을 실제 v22/same-PC/Tobii 병합 상태로 정정

현재 단계는 설계 기준 확정이며 Lua/Java 구현은 아직 v22 그대로다.
...
- `Agents_updated(260724).md`
- `Project_Context_updated(260724).md`
- `Worklog_updated(260724).md`
...
- 현재 delay는 `6~14초`, 다음 계획은 audio start 이후 `3~10초`이다.
- 현재 countdown/debug/advisory가 participant 화면에 보이며 다음 구현에서 숨긴다.
- 현재 response detector는 advisory 기준이며 다음 구현에서 spawn 기준으로 바꾼다.
...
- participant가 countdown/debug/advisory를 볼 수 있어 직접 발견 실험을 편향시킨다.
...
- 음성 유지과제, task event, 3~10초 독립 지연은 아직 구현되지 않았다.
- Spark의 3모니터 물리 coverage는 Surround validation 전에는 알 수 없다.
...
- 물리 `CENTER_BASELINE`과 NVIDIA Surround+ETM whole-display calibration 이후의 `SURROUND_WIDE` 실행은 아직 남아 있다.
- 실제 기준 측정 전 평소 비행 자세의 눈-중앙 화면 수직거리(mm)를 실측해야 한다.
