...
참가자가 visual advisory를 보고 반응하는 실험이 아니라 intruder를 직접 발견하고 회피하는 실험으로 재설계한다. v33부터 `INTRUDER_SPAWNED`는 객관적인 scenario onset으로 보존하고, 조명된 intruder가 화면 안에서 최소 5 px 추정 span을 갖는 `INTRUDER_VISUAL_OPPORTUNITY_ONSET`을 primary operational response anchor로 사용한다. 이 event는 실제 인지시점이 아니라 표준화된 관측기회 시작점이다. 기존 20 px `INTRUDER_VISUALLY_DETECTABLE`은 보조 진단값으로 유지한다.
...
v33~v36에서는 light-assisted visual-opportunity-to-response를 primary latency로 사용하고 spawn-to-response는 secondary scenario-exposure metric으로 유지한다. 20 px detectability-to-response는 표적이 더 뚜렷해진 뒤의 반응 여부를 살피는 보조 지표다. v34는 물리 시나리오와 반응 anchor는 유지하면서 세션 내 방향 순서를 균형화했고, v35는 bounded speed-stability spawn gate를 시험했다. v36은 v35 물리 검증 결과에 따라 raw IAS-rate를 필수 gate에서 제외하고 target-band dwell만 사용한다.
