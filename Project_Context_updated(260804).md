...
v33/v34에서는 light-assisted visual-opportunity-to-response를 primary latency로 사용하고 spawn-to-response는 secondary scenario-exposure metric으로 유지한다. 20 px detectability-to-response는 표적이 더 뚜렷해진 뒤의 반응 여부를 살피는 보조 지표다. v34는 물리 시나리오와 반응 anchor는 유지하면서 세션 내 방향 순서만 균형화한다.
