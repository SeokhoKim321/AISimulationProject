...
2. NVIDIA Surround와 Spark 한 대로 3모니터 15점 feasibility validation을 수행한다.
...
The software status is `software-ready / physical Surround test pending`.
No physical center-target baseline or Surround-wide gaze run has yet been used
for a conclusion.
...
After testing, restore extended desktop and the center `1920x1080` Tobii
display setup/calibration before ordinary X-Plane experiments. Until the
Surround pair passes, keep the center-only measurement and
`UNTRACKED_OR_OFF_DISPLAY` interpretation unchanged.
