...
- snapshot: `FLYWITHLUA_STUDY_INTEGRATED_260728_audio_anchored_spawn_v27.lua`
...
- 현재 표준 음성: trial 시작 `3초` 후 `6.493초` 고정 WAV 자동 재생
...
현재 v27 final approach crossing trial 흐름:

1. `TRIAL_RESET`
2. `TRIAL_START(protocol=audio_task_v27, sound ratios)`
3. trial start 3초 뒤 `TASK_COMMAND_AUDIO_START`
...
7. 조건 충족 시 `HAZARD_DETECTED`, `ADVISORY_SHOWN`
8. advisory 기준 `PILOT_RESPONSE_START`
9. `MIN_DISTANCE_REACHED`
10. `HAZARD_CLEARED`, `ADVISORY_CLEARED`
11. 필요 시 `PILOT_RESPONSE_END`
12. `TRIAL_END`
...
6. spawn 기준 response detector를 구현한다.
7. 기존 strict clean을 `recording/event complete`, `flight-condition-valid`, outcome으로 분리한다.
8. 먼 거리 intruder와 pixel/FOV 기반 `INTRUDER_DETECTABLE_ONSET`을 pilot test한다.
9. runway/intruder dynamic AOI와 external-other 분류를 추가한다.
10. 3~5회 pilot trial로 target envelope와 response/dwell threshold를 수동 검증한다.
11. protocol을 동결한 뒤 새 본실험 데이터를 수집한다. 기존 2026-07-20 자료와 합치지 않는다.
