...
참가자가 visual advisory를 보고 반응하는 실험이 아니라 intruder를 직접 발견하고 회피하는 실험으로 재설계한다. v32부터 `INTRUDER_SPAWNED`는 객관적인 threat-exposure onset으로 보존하고, 화면상 식별 가능 크기에 도달한 `INTRUDER_VISUALLY_DETECTABLE`을 primary operational response anchor로 사용한다. 이 detectability event도 조종사의 의식적 인지시점을 직접 측정한 값은 아니다.

주요 분석 대상:

- `INTRUDER_VISUALLY_DETECTABLE` 이후 `PILOT_RESPONSE_START`까지의 operational response latency
...
4. 실제 screenshot/pilot으로 최소 가시성 threshold 보정
5. threshold가 지속시간 조건을 만족한 첫 시점에
   `INTRUDER_DETECTABLE_ONSET` 기록
6. detectability-to-response를 주 후보, spawn-to-response를 보조지표로 유지
...
2. Surround coarse `SIDE_EXTERNAL` pilot은 현재 operator/setup에서 통과했으며 설정을 동결한다.
3. participant 화면 분리와 표준 WAV/task event는 v26에서 구현 완료했다.
4. 음성 시작 뒤 독립적인 `3~10초` 지연과 실제 최초 배치 기준 spawn event는 v27에서 구현 완료했다.
5. X-Plane에서 v27을 reload한 뒤 3회 timing pilot으로 planned/actual delay와 음량 context를 검증한다.
6. spawn 기준 response detector와 no-alert hazard/advisory 의미 분리를 구현 완료했다.
7. 새 receiver로 실제 1회 trial을 수행해 baseline event와 spawn-to-response를 검증한다.
8. 기존 strict clean을 `recording/event complete`, `flight-condition-valid`, outcome으로 추가 분리한다.
9. 먼 거리 intruder와 pixel/FOV 기반 `INTRUDER_DETECTABLE_ONSET`을 pilot test한다.
10. runway/intruder dynamic AOI와 external-other 분류를 추가한다.
11. 3~5회 pilot trial로 target envelope와 response/dwell threshold를 수동 검증한다.
12. protocol을 동결한 뒤 새 본실험 데이터를 수집한다. 기존 2026-07-20 자료와 합치지 않는다.
...
Static/synthetic verification passed. The 20 px value is not frozen. The next
required physical step is one short LEFT/RIGHT/FRONT calibration run after a
FlyWithLua reload, comparing the logged threshold crossing with what is
actually identifiable in the recorded 5760x1080 cockpit view.
