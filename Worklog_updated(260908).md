...
8월 25~26일 연구실 기술 파일럿 7명과 최신 experienced operator reference 1명의
수집·사후분석을 완료했다. primary는 자동 6-trial 완주자 6명의 36 trial이며, 수면
충돌로 종료된 1명의 4 trial은 sensitivity로 분리한다. 최신 reference는
`session_main_KIM_SEOKHO_20260826_164645`를 사용하고 8월 12일 self 세션은 반복
재현성 확인용으로만 보존한다.

벤치마크 논문의 attention-zone DTMC를 현재 X-Plane/Tobii AOI에 적용하는 전이확률
행렬 분석기를 구현했다. `200 ms` dominant AOI를 사용해 self-transition 포함 DTMC와
self-transition 제외 next-switch matrix를 trial phase별로 생성한다. 기존 v39 integrated
세션은 `FULL_TRIAL` 기준 transitions `816`, self `603 (73.9%)`, switches `213`으로
정상 산출됐고 probability row-sum과 SVG XML이 PASS했다. 연구실 구성원 1인 1회 절차도
automatic 6-trial 기준으로 작성했지만, 학위논문·학술발표용 사람 데이터 수집은 IRB 승인
또는 공식 면제 판단 이후에만 시행한다. 그 전에 `45 px` dynamic object margin을 동결한다.

이전 진행: v35 first physical validation에서 raw IAS-rate 필수조건 때문에 목표속도 범위에
들어온 trial 1과 5도 timeout이 된 것을 확인했다. v36에서는 목표 `±5 KIAS`를
`1.5 s` 연속 유지하는 target-band dwell만 생성 필수조건으로 사용하고 IAS
rate는 진단값으로 유지했다. 실제 v36 6-trial 결과 stable `4/6`, forced `2/6`
이며 forced 두 건은 모두 실제 목표오차 초과였다. v36 engineering validation은
PASS다. 8월 6일 그룹미팅 피드백에 따라 control intent 구분은 필수 연구목표에서
제외하고, intruder screen geometry 기반 opportunity와 grid/object 기반 외부시선
분류를 다음 구현 단계로 정했다.

현재 작업의 핵심:

- v33 physical screen/light/event/data merge 6-trial 검증 완료
- v33 retry session Java strict clean `6/6`, gaze 유효율 `84.8%`
- v33 방향표가 RIGHT-RIGHT-FRONT-FRONT-LEFT-LEFT로 묶여 trial order와 교란됨을 확인
- v34에서 trial 1~3과 4~6을 각각 LEFT/RIGHT/FRONT 1회씩 포함하는 균형 블록으로 변경
- 블록 경계 포함 인접한 동일 방향 반복 금지
- session/trial event에 schedule method와 block index 추가
- Java/Tobii `audio_task_v34` 호환 및 v33 회귀 검증 완료
- 저장소 Lua와 실제 X-Plane Scripts 실행본 hash 일치 확인
- behavior classifier v1 구현 및 smoke test 통과
- v33 retry 결과: speed-task correction 2, ambiguous 4, avoidance candidate 0
- v36 candidate 생성시각은 음성 시작 후 무작위 `6~10 s`
- 목표 `±5 KIAS` 범위를 `1.5 s` 연속 유지한 뒤 생성
- IAS rate는 event와 분석에 보존하지만 생성 필수조건에서는 제외
- 안정 미충족 시 음성 시작 `14 s`에 강제 생성하고 원인을 event에 기록
- Java/Python 분석에서 v35와 v36의 서로 다른 gate 규칙을 각각 교차검증
- v36 physical session Java strict clean 및 timing-valid `6/6`
- v36 stable `4/6`, forced `2/6`; forced error `+6.893/-7.835 KIAS`
- v36 raw gaze validity `82.4%`, behavior `AMBIGUOUS 4 / AVOIDANCE_CANDIDATE 2`
...
- `Agents_updated(260824).md`
- `Project_Context_updated(260824).md`
- `Worklog_updated(260824).md`
...
### 2026-08-11 v39 AI autopilot override light test

v38 physical observation 및 log 판정:

- 사용자 육안: 불빛이 계속 보임
- spawn 직후: beacon/nav/strobe/landing/taxi `0/0/0/0/0`
- 다음 frame pre-force: `0/0/0/1/1`
- 첫 opportunity까지 `lights_reenabled_count=333`
- 결론: v38 write는 성공했지만 X-Plane AI가 landing/taxi를 매 frame 복구했다.

v39 구현:

- active intruder에 한해 `override_plane_ai_autopilot[1]=1`을 매 frame 유지한다.
- retire, trial/session stop 및 script reload 초기화 시 override를 0으로 복구한다.
- 시작/기회 event에 override write/readback/value를 기록한다.
- protocol/profile을 `audio_task_v39` / `visual_opportunity_proxy_v39`로 갱신했다.
- Java/Tobii analyzer는 v39을 v38과 같은 timing/clean/response 계약으로 처리한다.
- Java 21 compile, Java auto-event/random-speed smoke, Python compile/analyzer/classifier
  smoke는 PASS했다.
- 다음 단계는 v39 reload 후 불빛과 궤적을 짧게 육안 검증하고 log에서 override=1,
  lights reenabled count를 확인하는 것이다.
- 기준본, v39 snapshot, 실제 Scripts 실행본 SHA-256은
  `6556B91F9E2AA568BE0C573261F1AE99BB09602505DB395206D36E1013F6BFE1`로 일치한다.

v39 single-trial physical 결과:

- approach: `LEFT`
- 사용자 육안: intruder 불빛이 보이지 않음
- override write/readback/value: `true/true/1`
- light readback beacon/nav/strobe/landing/taxi: `0/0/0/0/0`
- `lights_reenabled_count=0`; v39 reenabled marker 없음
- initial span: `13.819 px`
- spawn-to-opportunity: `5.452 s`
- opportunity span: `20.004 px`
- `MIN_DISTANCE_REACHED=100.111 m`, manual `TRIAL_END` 기록
- 판정: light suppression 및 intruder movement single-trial engineering PASS
- 후속 확인 항목: balanced 6-trial에서 FRONT/LEFT/RIGHT 재점등 0과 정상 궤적 검증

v39 balanced 6-trial final light validation:

| Session trial | Direction | Initial span px | Spawn→opportunity s | Override | Lights off | Reenabled | Min distance m |
|---:|---|---:|---:|---:|---|---:|---:|
| 1 | LEFT | 11.851 | 7.826 | 1 | true | 0 | 10.788 |
| 2 | RIGHT | 13.886 | 5.295 | 1 | true | 0 | 63.317 |
| 3 | FRONT | 8.337 | 10.814 | 1 | true | 0 | 7.298 |
| 4 | RIGHT | 13.774 | 5.366 | 1 | true | 0 | 22.233 |
| 5 | FRONT | 8.975 | 8.943 | 1 | true | 0 | 10.370 |
| 6 | LEFT | 12.992 | 6.893 | 1 | true | 0 | 28.471 |

- direction balance: LEFT/RIGHT/FRONT 각 2회
- spawn/opportunity/min-distance/trial-end: 각각 `6/6`
- automatic session complete: `1/1`
- reenabled marker: `0`
- Lua error: `0`
- 사용자 육안: 6-trial 모두 불빛 없음
- spawn-to-opportunity target `5~11 s`: `6/6`
- 최종 판정: v39 light suppression + intruder movement engineering PASS

### 2026-08-12 v39 integrated Java/Tobii 6-trial validation

원본:

- X-Plane session: `session_v39_integrated_20260812_155103`
- X-Plane prefix: `logs/xplane/xplane_v39_integrated_20260812_155103`
- Tobii gaze: `logs/tobii/session_xplane_tobii_20260812_155022_gaze.csv`
- rows: STATE `3247`, INTRUDER `3247`, EVENT `103`, gaze `30140`
- `RECEIVER_STOP=1`, `SESSION_STOP=1`; Java/Tobii 정상 종료

Recording 및 v39 light:

- direction schedule: `LEFT|RIGHT|FRONT|RIGHT|FRONT|LEFT`; 각 방향 2회
- Java clean: `6/6`, incomplete `0`
- spawn/opportunity/response start/response end/automatic end: 각각 `6/6`
- spawn 및 opportunity의 override readback/value: `true/1`, 각 `6/6`
- spawn 및 opportunity의 `lights_all_off=true`: 각 `6/6`
- positive `lights_reenabled_count`: `0`; 육안상 불빛 없음
- 결론: Java/Tobii 동시 기록 조건에서도 v39 light suppression과 자동 세션 진행 PASS

Geometry 및 response:

| Trial | Direction | Initial span px | Spawn→opportunity s | Opportunity→response s | Active at opportunity |
|---:|---|---:|---:|---:|---|
| 1 | LEFT | 13.701 | 5.618 | 4.228 | false |
| 2 | RIGHT | 12.820 | 7.050 | 3.447 | false |
| 3 | FRONT | 8.419 | 10.552 | 2.258 | false |
| 4 | RIGHT | 12.905 | 6.452 | 10.199 | false |
| 5 | FRONT | 8.050 | 10.921 | 1.631 | true |
| 6 | LEFT | 12.096 | 7.639 | 7.457 | false |

- spawn-to-opportunity target `5~11 s`: `6/6`; 평균 `8.039 s`
- provisional initial span target `7~13 px`: `5/6`; trial 1은 `13.701 px`
- opportunity-to-response 평균/중앙값: `4.870/3.838 s`
- trial 5는 opportunity 당시 control active이므로 latency 해석 주의
- hazard detect/clear는 `4/4`; 현재 primary latency에는 hazard/advisory를 사용하지 않음

Speed gate:

- stable: trial `2,3,4,5` = `4/6`
- forced timeout: trial `1,6` = `2/6`
- trial 1: target `90`, spawn error `+14.037 KIAS`; primary 분석 제외 후보
- trial 6: target `105`, spawn error `+3.745 KIAS`, dwell `1.357 s`; band 안이지만
  1.5 s dwell 미달로 forced
- recording clean과 speed-condition eligibility를 분리한다. primary는 stable trial,
  forced 포함 결과는 sensitivity로 보고한다.

Tobii 및 AOI:

- raw valid: `21207/30140 = 70.4%`
- trial valid: `83.8~92.6%`; trial 동일가중 평균 `87.6%`
- trial operational-external 평균: `49.3%`
- trial 전체 표본의 주요 operational AOI: external `49.2%`, AIRSPEED `16.0%`,
  VERTICAL_SPEED `11.4%`, unresolved tracking loss `8.3%`
- visual opportunity 이후 첫 operational-external 평균/중앙값: `0.307/0.194 s`
- 외부시선은 intruder acquisition 확정값이 아니며, opportunity 이전부터 외부를 본
  trial도 `3/6`이므로 의미 분류가 다음 과제다.

Behavior diagnostic:

- `AMBIGUOUS 5 / AVOIDANCE_CANDIDATE 1`
- classifier는 목적 확정값이 아니고 교수 피드백상 primary outcome에서 제외한다.
- primary metric은 geometry opportunity 이후 최초 지속 조작 변화 시간이다.

생성 산출물:

- `build_atc_tmp/xplane_v39_integrated_20260812_155103_gaze_aoi.csv`
- `build_atc_tmp/xplane_v39_integrated_20260812_155103_external_episodes.csv`
- `build_atc_tmp/xplane_v39_integrated_20260812_155103_trial_gaze_summary.csv`
- `build_atc_tmp/xplane_v39_integrated_20260812_155103_trial_aggregate_summary.csv`
- `build_atc_tmp/xplane_v39_integrated_20260812_155103_trial_behavior_summary.csv`

판정 및 다음 단계:

- v39 integrated engineering validation: PASS
- 이번 세션만으로 20 px threshold나 실험효과를 확정하지 않는다.
- v39은 활성 실험 build로 동결하고, 다음은 정면 grid + projected intruder dynamic AOI +
  runway region 기반 외부시선 목적 evidence를 구현한다.

### 2026-08-19 external gaze context classifier 구현 및 검증

구현 파일:

- classifier: `tools/tobii/classify_xplane_external_gaze.py`
- smoke test: `tools/tobii/classify_xplane_external_gaze_smoke_test.py`
- runway geometry: `resources/rksi_runway34_geometry_260819.csv`
- procedure: `TOBII_EXTERNAL_GAZE_CLASSIFICATION_260819.md`
- classifier version: `260819_dynamic_external_context_v1`

설계:

- 원본 STATE/INTRUDER/EVENT/gaze 및 UDP/CSV schema는 변경하지 않는다.
- 기존 `*_gaze_aoi.csv`를 입력으로 받는 별도 사후분석 단계다.
- spatial layer: LEFT/CENTER/RIGHT monitor별 `6 x 3` grid
- intruder object layer: gaze timestamp에 가장 가까운 state와 같은 sample index의
  intruder를 결합하고 Lua와 동일한 perspective projection 계산
- runway object layer: RKSI `apt.dat` runway 34 양 끝점, width `60.05 m`, airport
  elevation `20 ft = 6.096 m`를 local coordinates와 화면 polygon으로 변환
- semantic layer: `INTRUDER_SEARCH_OR_TRACKING`, `RUNWAY_GUIDANCE`,
  `OTHER_EXTERNAL`, `UNRESOLVED`
- 기본 intruder/runway margin: `45/45 px`; dwell `200 ms`
- evidence priority: dynamic intruder, dynamic runway, same intruder grid,
  post-spawn measured side-direction match, residual external
- direct dynamic overlap, grid match, side match를 별도 보존
- label은 awareness/recognition/intent 확정값이 아니라 operational proxy

v39 actual integrated result:

- source: `xplane_v39_integrated_20260812_155103`
- trial 내부 operational-external rows: `5944`
- intruder: `1478 (24.9%)`
- runway: `298 (5.0%)`
- other external: `2845 (47.9%)`
- unresolved: `1323 (22.3%)`
- intruder evidence breakdown: direct dynamic `879`, grid `438`, measured side `161`
- projection median absolute error: x `0.083 px`, y `0.363 px`, span `0.026 px`

v37 backward actual-session check:

- external rows: `6151`
- intruder/runway/other/unresolved: `24.2/6.4/43.3/26.1%`
- projection median absolute error: x `0.233 px`, y `0.724 px`, span `0.025 px`

Margin sensitivity, v39:

| Margin | Intruder | Runway | Other | Unresolved |
|---:|---:|---:|---:|---:|
| 30 px | 22.9% | 2.0% | 56.7% | 18.4% |
| 45 px | 24.9% | 5.0% | 47.9% | 22.3% |
| 60 px | 27.9% | 11.0% | 35.0% | 26.1% |

검증:

- new synthetic smoke: PASS
- all Tobii Python compile: PASS, `13` files
- existing gaze analyzer smoke: PASS
- existing behavior classifier smoke: PASS
- v39/v37 actual projection cross-check: PASS
- output SVG XML: PASS; 6 trajectories, 6 runway polygons 포함
- `git diff --check`: PASS

v39 기본 산출물:

- `build_atc_tmp/xplane_v39_integrated_20260812_155103_external_context_gaze.csv`
- `build_atc_tmp/xplane_v39_integrated_20260812_155103_external_semantic_episodes.csv`
- `build_atc_tmp/xplane_v39_integrated_20260812_155103_external_semantic_summary.csv`
- `build_atc_tmp/xplane_v39_integrated_20260812_155103_external_grid_summary.csv`
- `build_atc_tmp/xplane_v39_integrated_20260812_155103_projection_validation.csv`
- `build_atc_tmp/xplane_v39_integrated_20260812_155103_external_context_metadata.csv`
- `build_atc_tmp/xplane_v39_integrated_20260812_155103_external_context_overview.svg`

판정:

- spatial/object projection engineering implementation: PASS
- semantic classifier implementation: complete
- semantic accuracy 및 `45 px` margin: provisional, 수동검증 후 freeze
- 다음 단계: runway/intruder/other를 의도적으로 보는 짧은 검증 또는 화면 녹화와
  동적 AOI overlay 비교

### 2026-08-24 AOI transition matrix 및 연구실 참가자 절차

벤치마크 확인:

- local source: `CognitiveAgentEvaluationforSyntheticPilotTraining.pdf`
- reference: Hanak, Novak, Chudy, DASC 2025, Eq. 3~4, Fig. 6
- attention model: source zone `i`에서 next zone `j`로 가는 DTMC
  `P(s_(t+1)=j | s_t=i)`
- 각 source row probability 합은 1
- diagonal self-transition은 같은 attention zone에 계속 머무는 duration/persistence를
  모델링
- reference paper는 tactical state별로 별도 sparse transition matrix 사용

구현:

- analyzer: `tools/tobii/analyze_aoi_transition_matrix.py`
- smoke: `tools/tobii/analyze_aoi_transition_matrix_smoke_test.py`
- analyzer version: `260824_aoi_transition_matrix_v1`
- input: `*_external_context_gaze.csv` + matching `_events.csv`
- raw STATE/INTRUDER/EVENT/gaze 및 CSV schema 변경 없음
- default observation: `200 ms` bin의 dominant eligible AOI
- fixation detector를 사용하지 않으므로 결과를 fixation으로 부르지 않음
- invalid/unresolved bin은 transition chain을 끊고 앞뒤 state를 연결하지 않음
- AOI: six instruments + `PANEL_OTHER`, `INTRUDER_AOI`, `RUNWAY_AOI`,
  `OTHER_EXTERNAL`
- phase: `FULL_TRIAL`, `BEFORE_SPAWN`, `SPAWN_TO_OPPORTUNITY`,
  `AFTER_OPPORTUNITY`
- DTMC: self-transition 포함, benchmark paper 대응
- next-switch: self-transition 제외, 실제 다른 AOI로 옮긴 조건부확률
- multi-participant: participant별, pooled-transition, participant-equal mean을 함께 출력
- 기본 SVG는 participant-equal mean 사용
...
- `*_dtmc_probability_matrix.csv/.svg`
- `*_switch_count_matrix.csv`
- `*_switch_probability_matrix.csv/.svg`
- `*_summary.csv`
- `*_metadata.csv`
...
- source: `xplane_v39_integrated_20260812_155103`
- binned states: `1745` across full and three phase-specific copies
- DTMC transitions: `1617`
- AOI switches: `420`
- `FULL_TRIAL`: eligible bins `873`, transitions `816`, self `603`, switches `213`
- full-trial self-transition rate: `73.9%`
- phase self-transition rate: before spawn `74.8%`, spawn-to-opportunity `72.6%`,
  after opportunity `74.5%`
- full-trial next-switch top examples: `VERTICAL_SPEED -> AIRSPEED 57.1%`,
  `INTRUDER_AOI -> OTHER_EXTERNAL 54.5%`, `ALTITUDE -> VERTICAL_SPEED 46.2%`
- 이 결과는 self 1인의 engineering session 기술값이며 집단 조종사 결과가 아님

검증:

- synthetic matrix/phase/gap/dwell smoke: PASS
- probability row sum: PASS
- Python compile: PASS
- DTMC/switch SVG XML: PASS

연구실 참가자 준비:

- Tobii logger에 optional `--participant-code`, `--duration-s`, `--project-root` 추가
- participant code가 없으면 기존 `session_xplane_tobii_<timestamp>` 이름 유지
- gaze CSV column schema 변경 없음
- protocol: `AOI_TRANSITION_MATRIX_AND_LAB_PILOT_PROTOCOL_260824.md`
- manifest template: `resources/aoi_transition_session_manifest_template_260824.csv`
- 각 participant는 동일 saved situation에서 automatic balanced 6-trial session 1개
- LEFT/RIGHT/FRONT 각 2회, participant keypress는 session start 1회만 사용
- IRB 승인/면제 판단 전 학위논문·학술발표용 연구실 구성원 data 수집은 시작하지 않음
- 비조종 연구실 구성원은 feasibility/novice sample이며 pilot 대표값으로 해석하지 않음
- main collection 전 `45 px` dynamic object margin과 exclusion rule 동결 필요

### 2026-08-11 v38 persistent intruder lights-off 진단 보완

사용자 육안 확인에서 v37의 intruder 점멸등이 계속 보였다. 기존
`lights_write_ok=true`를 실제 화면 소멸의 증거로 사용하지 않기로 했다.

구현:

- profile을 `visual_opportunity_proxy_v38`로 갱신했다.
- head-on 및 crossing intruder active 전 구간에 매-frame lights OFF 강제를 추가했다.
- beacon/nav/strobe/landing/taxi 값을 write 직후 readback하고, 다음 frame 시작 시 값이
  다시 켜졌는지 검사한다.
- spawn/opportunity detail에 `lights_persistent_force`, `lights_readback_ok`,
  `lights_all_off`, `lights_reenabled_count`, 개별 readback 값을 기록한다.
- simulator가 frame 사이에 light를 다시 켜면 최초 1회
  `intruder_lights_reenabled_by_sim=true` marker를 기록한다.
- AI 이동을 중단시킬 위험이 있는 `override_planepath`는 사용하지 않았다.

분석/검증:

- `XPlaneSessionAnalysisMain`은 v38을 v37과 같은 visual-opportunity 및
  target-band-dwell 계약으로 처리한다.
- Tobii analyzer version은 `260811_persistent_intruder_lights_off_v13`이다.
- Java 21 compile, auto-event smoke, random-speed smoke, Python analyzer/classifier
  smoke와 Python compile은 PASS했다.
- v37 실제 6-trial session backward 재분석은 Java clean `6/6`, trial gaze validity
  `85.4~92.7%`를 재현했다.
- local Lua interpreter가 없어 Lua runtime 최종 확인은 FlyWithLua reload로 수행한다.

다음 판정은 v38 reload 후 1개 intruder를 육안 확인하고 X-Plane `Log.txt`의 readback과
reenabled marker를 함께 보는 방식이다. readback이 0인데도 점멸하면 no-light AI
model/custom object 경로로 전환한다.

배포:

- 기준본과 `FLYWITHLUA_STUDY_INTEGRATED_260811_persistent_intruder_lights_off_v38.lua`
  snapshot을 생성했다.
- 실제 X-Plane Scripts의 `FLYWITHLUA_STUDY_INTEGRATED.lua`에 배포했다.
- 세 파일 SHA-256: `59EF750687D4C400335C4632463C79D9089A258FDD2AA801CDDCB2D7326760DE`
- 실제 Scripts 폴더 활성 Lua 수: `1`
- UDP target: `127.0.0.1:9100`

### 2026-08-04 v34 균형 방향 블록 구현

- `study_build_balanced_approach_schedule()`를 추가했다.
- 첫 블록과 둘째 블록은 각각 LEFT/RIGHT/FRONT의 무작위 순열이다.
- 둘째 블록 첫 방향이 첫 블록 마지막 방향과 같으면 둘째 블록을 다시 추첨한다.
- 가능한 24개 스케줄을 열거한 정적 검증에서 블록별 균형과 5개 인접 경계의 비반복 조건이 모두 통과했다.
- `automatic_session=start`에는 method, block 1, block 2, 전체 schedule을 기록한다.
- `trial_ready` 및 `TRIAL_START`에는 method와 `approach_block_index=1|2`를 기록한다.
- Lua version/protocol/profile을 각각 `260804_balanced_direction_blocks_v34`, `audio_task_v34`, `visual_opportunity_proxy_v34`로 갱신했다.
- Java visual-opportunity profile 판정은 version prefix를 사용해 v33/v34를 모두 지원한다.
- Java/Tobii clean 및 audio timing 판정에 v34를 추가했다.
- 검증: Java 21 compile PASS, `XPlaneAutoEventDetectorSmokeTest` PASS, `XPlaneRandomSpeedTaskAnalysisSmokeTest` PASS, Python 3.10 compile PASS.
- v33 실제 세션 회귀: Java `6/6 clean`; Tobii gaze `21199`, valid `17970 (84.8%)`로 기존 결과를 재현했다.
- snapshot: `FLYWITHLUA_STUDY_INTEGRATED_260804_balanced_direction_blocks_v34.lua`
- repository/deployed hash: `6AD4CA82B85997EEB6208824EC4ABC26405084810DDDFABC5E23897E0E0C6865`
- FlyWithLua runtime reload에서 v34 load PASS, Lua error/quarantine 없음.
- 현장 생성 schedule: `FRONT|LEFT|RIGHT|LEFT|RIGHT|FRONT`.
- block 1 `FRONT|LEFT|RIGHT`, block 2 `LEFT|RIGHT|FRONT`로 두 블록 모두 균형 조건을 만족했다.
- 모든 인접 trial의 방향이 달라 비반복 조건을 만족했다.
- session은 initial wait `12.692 s` 중 `9.646 s`에 중단했고, trial 생성 없이 `state=INITIAL_WAIT;completed_trials=0`로 정상 종료됐다.
- v34 방향 스케줄 단계는 완료했다.

### 2026-08-04 behavior-evidence classifier v1 구현

- 새 도구: `tools/tobii/classify_xplane_tobii_behavior.py`
- 새 smoke: `tools/tobii/classify_xplane_tobii_behavior_smoke_test.py`
- classifier version: `260804_behavior_evidence_v1`
- state/intruder/event CSV와 Tobii trial gaze summary를 결합한다.
- raw logging schema와 기존 `clean` 판정은 수정하지 않았다.
- 반응 전후 목표속도 오차 변화로 speed-task-consistent evidence를 계산한다.
- ownship/intruder 상대 위치·속도로 반응 전후 projected 3-D CPA를 계산한다.
- projected CPA 개선만으로 회피라고 하지 않고 response 전 operational-external evidence와 active-control flag를 함께 요구한다.
- 출력 class: `SPEED_TASK_CORRECTION`, `AVOIDANCE_CANDIDATE`, `AMBIGUOUS`, `NO_VALID_RESPONSE`.
- speed와 avoidance evidence가 동시에 있으면 dual-purpose `AMBIGUOUS`로 보존한다.
- Python compile 및 classifier smoke test: PASS.
- 실제 v33 retry output:
  `build_atc_tmp/xplane_visual_opportunity_v33_retry_20260804_134141_trial_behavior_summary.csv`
- trial 3, 6: `SPEED_TASK_CORRECTION`
- trial 1, 2: specific-purpose evidence 부족 `AMBIGUOUS`
- trial 4, 5: dual speed/avoidance evidence `AMBIGUOUS`
- 명확한 `AVOIDANCE_CANDIDATE`: `0/6`
- trial 6은 CPA 개선이 있었지만 control response가 operational-external보다 먼저여서 avoidance evidence에서 제외했다.
- 결론: v33 retry는 6/6 recording clean이지만 behavior-valid avoidance dataset으로는 부족하다.
- v1 threshold는 자기실험 1세션 검사용 provisional heuristic이다. participant pilot/manual video review와 sensitivity analysis 전에는 논문 확정 cutoff로 사용하지 않는다.
...
- `tools/tobii/classify_xplane_external_gaze.py`
- `tools/tobii/analyze_aoi_transition_matrix.py`
- `tools/tobii/classify_xplane_tobii_behavior.py`
- `tools/tobii/classify_xplane_tobii_behavior_smoke_test.py`
...
- recording clean과 speed-condition eligibility는 분리했지만 v39 integrated session에서
  speed gate forced trial이 `2/6`이었다.
...
- 20 px visual opportunity와 dynamic runway/intruder AOI는 구현됐지만 실제 인지시점을
  뜻하지 않는다. 특히 `45 px` gaze-to-object margin은 아직 provisional이다.
- AOI transition matrix는 self 1인 자료에서만 engineering 검증됐으며 participant 집단
  결과가 아직 없다.
- 학위논문·학술발표용 연구실 구성원 데이터 수집은 IRB 승인 또는 공식 면제 판단 전에는
  시작하지 않는다.
- 비조종 연구실 구성원 결과는 pilot 모집단 대표자료로 사용할 수 없다.
- `plane1_*` intruder dataref writable 동작은 현재 X-Plane 11 환경에 의존한다.

## 다음 작업 계획

1. 학위논문과 관련 학술발표 활용을 포함한 IRB 신규 심의 또는 공식 면제 판단
2. controlled target-gaze validation으로 `45 px` object margin 동결
3. IRB 승인 이후 연구실 구성원 1인당 automatic 6-trial session 1회 수집
4. session별 merge/external context 분석과 acceptance check
5. exclusion rule을 자료 확인 전에 동결하고 manifest 작성
6. participant별/pooled/participant-equal AOI transition matrix 생성
7. 비조종자 feasibility와 자격·경험 조종사 결과를 구분해 보고
8. protocol 동결 후 본실험
...
## 2026-07-30 spawn 비행조건 기록 v30

구현:

- Lua version: `260730_spawn_flight_context_v30`
- task protocol: `audio_task_v29` 유지
- snapshot:
  `FLYWITHLUA_STUDY_INTEGRATED_260730_spawn_flight_context_v30.lua`
- 실제 X-Plane FlyWithLua `Scripts/FLYWITHLUA_STUDY_INTEGRATED.lua`에 배포
- Git 기준본, snapshot, runtime Lua SHA-256:
  `AF1F8B1553FCA1555D4F9F0025EC580D1366033A796B3FA5FCD8B0CCE853326E`

새 `INTRUDER_SPAWNED` detail:

- ownship latitude/longitude
- elevation/AGL
- heading/pitch/roll
- P/Q/R
- IAS/TAS/vertical speed
- pitch/roll/yaw input과 throttle
...
- 수정:
  ...
  - `src/com/example/ai/XPlaneBatchAnalysisMain.java`
  - `src/com/example/ai/XPlaneRandomSpeedTaskAnalysisSmokeTest.java`
- raw CSV schema는 변경하지 않음
- 새 event detail이 있으면 정확한 event 값을 사용
- 과거 로그는 spawn sim time과 가장 가까운 STATE row로 복원
- session console에 spawn position/attitude/controls 출력
- batch trial/clean-trial CSV에 spawn context 컬럼 추가
- flap은 추가하지 않음
...
- Java 21 compile 통과
- v29 random-speed analysis smoke 통과
- 2026-07-29 group-meeting v29 세션 회귀분석 통과
- 과거 trial 28~40에서 spawn 위치·AGL·자세·입력이 정상 복원됨
- batch CSV spawn context header/value 검증 통과
- 로컬 `luac`가 없어 Lua는 정적 version/helper check만 수행
- 실제 X-Plane 메모리 활성화와 Lua syntax/runtime 확인에는 FlyWithLua 전체
  reload가 필요

회피/속도조절 구분 설계:

- `SPEED_TASK_CONTROL`: throttle 중심 변화와 IAS target error 감소, 실제
  trajectory/predicted separation 변화는 작음
- `AVOIDANCE_CANDIDATE`: 지속적인 roll/pitch/heading/vertical path 변화와
  predicted closest-approach separation 개선
- `AMBIGUOUS`: `active_control_at_spawn=true` 또는 두 유형 중첩
- gaze는 동적 intruder AOI/외부 전환을 보조근거로 사용
- 자동 분류는 대표 영상의 수동 label과 비교 검증

계속 필요한 작업:

1. 실시간 attitude stability gate와 자동 6-trial session state machine
2. screen projection/pixel size 기반 `INTRUDER_DETECTABLE_ONSET`
3. `INTRUDER_DYNAMIC`, `RUNWAY_DYNAMIC`,
   `HORIZON_OR_EXTERNAL_OTHER` AOI
4. predicted separation 기반 행동 분류와 수동 영상 검증

## 2026-08-03 v30 실제 spawn 비행조건 검증

파일:

- session: `session_spawn_context_v30_20260803_150206`
- prefix: `logs/xplane/xplane_spawn_context_v30_20260803_150206`
- analysis-time rows: STATE `290`, INTRUDER `290`, EVENT `15`

결과:

- X-Plane `Log.txt`: `260730_spawn_flight_context_v30` 정상 load
- trial: right approach, `1/1 CLEAN`
- audio-to-spawn planned/actual: `3.909/3.911 s`
- spawn AGL: `565.859 m`
- heading/pitch/roll: `325.030/-1.120/-1.148 deg`
- IAS/TAS/vertical speed: `116.620 KIAS`, `62.044 m/s`, `-0.465 m/s`
- pitch/roll/yaw/throttle input:
  `-0.3454/0.0059/-0.0027/1.0000`
- Lua event detail과 Java trial summary가 일치함

판정:

- v30 spawn flight-context logging 물리 검증 통과
- 다음 구현은 session start 1회 기반 자동 6-trial state machine,
  방향/속도 schedule, washout과 실시간 자세 안정 gate

## 2026-08-03 automatic six-trial session v31 implementation

Implemented:

- Lua/version: `260803_auto_six_trial_session_v31`, protocol `audio_task_v31`
- snapshot: `FLYWITHLUA_STUDY_INTEGRATED_260803_auto_six_trial_session_v31.lua`
- commands/macros: `Study Start Automatic Session`,
  `Study Stop Automatic Session`
- one session-start action automatically runs six trials
- direction schedule: LEFT/RIGHT/FRONT exactly twice each, shuffled per session
- speed schedule: 90/95/100/105/110 KIAS once each plus one random repeat,
  shuffled per session
- initial wait and inter-trial washout: independent `U(10,15 s)`
- real-time 3-second stability gate: absolute roll <= 7 deg, roll range <= 3
  deg, pitch range <= 3 deg, heading range <= 5 deg
- unstable flight extends the wait; it does not force-start the next trial
- automatic close: existing close guard plus >=2 seconds after predicted
  conflict and current horizontal separation >=200 m
- intruder retired below the visible scene at trial close/automatic stop
- participant display hides schedule/countdown and shows only minimal progress
- raw CSV schemas unchanged; automatic-session metadata uses EVENT detail
- Java session analyzer and Tobii merge analyzer updated for `audio_task_v31`
- current-protocol Java smoke test updated to v31

Verification:

- Java 21 compile: PASS
- `XPlaneRandomSpeedTaskAnalysisSmokeTest`: PASS
- Python 3.10 compile for `analyze_xplane_tobii_session.py`: PASS
- `git diff --check`: PASS
- repository/snapshot/runtime Lua hash match:
  `92FADE00DC2BC9AC0BBD3960ED442D7064AD28CB1C8B23BCBC5AE1EC6EEEC9EE`

Current handoff:

- v31 has been copied to the actual X-Plane FlyWithLua Scripts directory.
- X-Plane FlyWithLua reload confirmed
  `260803_auto_six_trial_session_v31` loaded with no FlyWithLua error.
- The first complete six-trial pilot was run and is documented below.

## 2026-08-03 v31 full automatic-session physical pilot

Files:

- session: `session_auto_session_v31_20260803_152309`
- prefix: `logs/xplane/xplane_auto_session_v31_20260803_152309`
- final rows: STATE `1746`, INTRUDER `1746`, EVENT `95`
- clean receiver shutdown: `RECEIVER_STOP`, `SESSION_STOP`, UDP 9100 released

Automatic-session result:

- duration: `212.320 s`
- trials: `6/6 strict clean`
- direction order: RIGHT, LEFT, RIGHT, LEFT, FRONT, FRONT
- direction balance: RIGHT 2, LEFT 2, FRONT 2
- speed order: 90, 100, 105, 95, 90, 110 KIAS
- speed rule: all five targets once plus a repeated 90 KIAS target
- six occurrences each of audio start/end, spawn, hazard detect/clear,
  response baseline/start/end, minimum distance, and automatic trial end
- planned waits: 11.94, 13.93, 10.98, 10.44, 11.20, 11.66 s
- actual waits: 11.94, 13.93, 12.99, 10.44, 11.22, 12.23 s
- stability gate passed for every trial; trials 3 and 6 correctly waited longer
- minimum-distance to automatic-end delay: approximately 2.0 s for all trials

Analyzer defect found and fixed:

- Lua retires the intruder below the scene after `TRIAL_END`, but subsequent
  washout packets retain the previous trial id.
- The previous analyzer included these rows in trial distance/control metrics,
  yielding a false near-zero horizontal distance with about 3000 m vertical
  separation.
- Raw CSV files were not changed.
- `XPlaneSessionAnalysisMain` now bounds STATE/INTRUDER rows to the inclusive
  `TRIAL_START..TRIAL_END` time window for trial metrics.
- Session minimum separation is derived from bounded trial summaries.
- `XPlaneRandomSpeedTaskAnalysisSmokeTest` now injects post-trial retirement
  rows and asserts that they are excluded.
- Java 21 compile, regression smoke, and current-session reanalysis: PASS.

Correct bounded minimum horizontal/vertical-at-minimum values:

- trial 1: `24.277 / 22.380 m`
- trial 2: `12.877 / 22.321 m`
- trial 3: `7.549 / 17.205 m`
- trial 4: `8.617 / 17.100 m`
- trial 5: `8.239 / 8.618 m`
- trial 6: `13.902 / 29.208 m`

Design findings before main collection:

- spawn AGL sequence: 487.9, 311.2, 227.2, 164.1, 103.3, 59.3 m
- session start/completion AGL: approximately 534 / 75 m
- final trial target/actual spawn speed: `110 / 81.3 KIAS`
- the late-session speed command is strongly confounded with approach phase
- trial 4 had `active_control_at_spawn=true`
- trials 2, 4, and 6 had raw-input response latencies below `0.2 s`, so the
  current response event cannot yet be interpreted as avoidance onset
- next design decision: constrain the usable approach envelope and/or make
  target-speed assignment phase-aware before the Tobii-inclusive main pilot

### Initial-condition correction before design change

- The first v31 pilot was not started from the saved 2100 ft scenario.
- Automatic-session start was approximately 1753 ft AGL after additional
  flight progress, and the pilot reported an excessive descent tendency.
- The low sixth-spawn altitude therefore cannot be attributed to the saved
  initial condition alone.
- Relative to the first pilot, an exact 2100 ft start adds about 347 ft; a
  simple same-path estimate places the sixth spawn near 542 ft AGL.
- Decision: do not change Lua, washout, six-trial count, or the 90-110 KIAS
  target set yet.
- Next test: reload the exact saved 2100 ft scenario and repeat one Java-only
  automatic six-trial session, while maintaining a controlled descent.

### Saved-scenario preflight diagnostic

- diagnostic prefix:
  `logs/xplane/xplane_auto_session_v31_2100ft_retest_20260803_154740`
- first logged altitude: approximately 2045 ft AGL
- after the requested short unpause, actual logged duration was about 8 s and
  altitude was approximately 1910 ft AGL
- classification: diagnostic/pre-roll only; do not count as a repeat session
- corrected clean-start sequence:
  1. load the saved scenario
  2. reload FlyWithLua
  3. start Java receiver while X-Plane remains paused
  4. press `Study Start Automatic Session` while paused
  5. unpause and fly; no separate pre-roll

## 2026-08-03 exact-start v31 clean repeat
...
- receiver stop: clean (`RECEIVER_STOP`, `SESSION_STOP`)

Automatic-session verification:

- `6/6 strict clean`
- direction order: LEFT, LEFT, RIGHT, FRONT, FRONT, RIGHT
- speed order: 95, 100, 105, 110, 90, 110 KIAS
- all six trials contain reset/start, audio start/end, spawn, hazard
  detect/clear, response baseline/start/end, minimum distance, and automatic
  trial end
- waits: 12.666, 12.767, 12.994, 14.926, 12.120, 10.737 seconds
- all recorded stability-gate values passed
- bounded minimum horizontal distance by trial: 4.809, 19.747, 11.510,
  6.178, 7.144, 35.694 m

Flight-state check:

- first STATE altitude: approximately 2066 ft AGL
- trial 6 end altitude: approximately 957 ft AGL
- whole-file pitch range: `-7.50..9.36 deg`
- whole-file vertical-speed range: approximately `-1143..1363 ft/min`
- the previous abnormal near-vertical dive did not recur
- Java remains receive-only and v31 Lua does not write ownship pitch, trim, or
  position; the invalid run is equipment/saved-state data and is excluded

Experimental-validity findings:

- spawn IAS: 99.970, 94.898, 98.646, 106.235, 107.849, 91.478 KIAS
- trials 5/6 target error: approximately 17.8/18.5 KIAS after a
  110 -> 90 -> 110 command sequence with about 5.9/5.7 seconds before spawn
- spawn-to-response: 1.413, 0.374, 7.887, 0.131, 0.043, 0.065 seconds
- four trials had active control at spawn
- current throttle/raw-input detector remains confounded with the speed task
- strict clean confirms event/log completeness, not speed-task success or
  behaviorally valid avoidance onset
- zero advisory events are expected under the intruder-spawn-anchored protocol

Conclusion:

- v31 six-trial automation, schedule balance, wait/stability gate, automatic
  closure, logging, and the 2100 ft start envelope are functionally validated.
- The next required implementation/design task is separating concurrent
  speed-control input from a genuine avoidance response before primary
  response-latency analysis or main participant collection.

## 2026-08-03 visual detectability gate v32 implementation

Implementation:

- Lua/version: `260803_visual_detectability_gate_v32`
- protocol: `audio_task_v32`
- snapshot:
  `FLYWITHLUA_STUDY_INTEGRATED_260803_visual_detectability_gate_v32.lua`
- repository/snapshot/runtime SHA-256:
  `7F6902AAA3480708CB0EC09523DD26B594FA5679850375AD6FCEC3C1681CEC71`
- v31 automatic six-trial state machine retained
- crossing target ahead: 920-1100 m
- configured time to conflict: 16-20 s
- spawn extra path distance: 500 m
- nominal intruder path-start distance: about 1.4-1.7 km
- maximum crossing duration: 50 s

Visual projection proxy:

- fixed display: 5760x1080, horizontal FOV 122 deg
- reference model: wingspan 11.0 m, length 8.3 m
- method: `perspective_bbox_proxy_v1`
- provisional threshold: 20 px
- heading/pitch included; roll correction not yet included
- spawn event logs initial projection state
- first visible threshold crossing records
  `INTRUDER_VISUALLY_DETECTABLE` exactly once
- detail includes span, screen x/y, slant/horizontal distance,
  bearing/elevation, threshold/display/FOV, spawn-to-detectability, and
  ownship context

Nominal geometry checks across 45-60 m/s ownship speed:

- LEFT/RIGHT initial span: approximately 12.0-14.4 px
- LEFT/RIGHT threshold crossing: approximately 4.7-8.0 s after movement start
- FRONT initial span: approximately 6.4-7.4 px
- FRONT threshold crossing: approximately 9.9-13.4 s after movement start
- the threshold is therefore not crossed at nominal spawn, unlike the rejected
  14 px provisional candidate

Java response/analysis changes:

- enum added: `INTRUDER_VISUALLY_DETECTABLE`
- hazard window still opens at spawn
- v32 response baseline/start opens at visual detectability
- legacy spawn anchor retained for v31 and older data
- response method:
  `pre_detectability_mean_sustained_delta_v2`
- baseline stores active control at the selected anchor and, for v32,
  `active_control_at_detectability`
- throttle remains a valid response axis
- v32 strict completeness requires one ordered detectability event
- analyzer and batch CSV include detectability geometry and latencies
- pre-gate response is labelled `RESPONSE_BEFORE_DETECTABILITY`

Tobii changes:

- analyzer version: `260803_visual_detectability_v8`
- `audio_task_v32` clean-order support
- detectability-relative operational-external windows, AOI entry/dwell/gaze,
  response latency, and pre-event state fields
- visualization default event changed to
  `INTRUDER_VISUALLY_DETECTABLE`
- historical anchors remain selectable with `--event`

Verification:

- Java 21 compile: PASS
- `XPlaneAutoEventDetectorSmokeTest`: PASS, including v32 gate anchor
- `XPlaneRandomSpeedTaskAnalysisSmokeTest`: PASS
- v31 six-trial backward analysis: `6/6 clean`
- Java batch detectability export: PASS
- Python 3.10 compile: PASS
- historical v29 Tobii backward analysis: PASS
- `git diff --check`: PASS
- active Lua copied to the actual X-Plane Scripts directory with matching hash

Pending physical validation:

1. Reload FlyWithLua and confirm v32 loads without error.
2. Run a short LEFT/RIGHT/FRONT calibration, preferably with screen recording.
3. Compare the visible aircraft at the logged 20 px crossing with actual
   identifiability.
4. Adjust/freeze the threshold only after that comparison.
5. Run the first full v32 six-trial Java/Tobii pilot after calibration.

## 2026-08-04 v33 light-assisted visual-opportunity 구현

요청/판단:

- intruder의 위치표시등 때문에 참가자 음성 마커보다 표준화된 표시 조건을
  response 기준으로 사용하는 방향을 채택했다.
- 실제 인지시점은 직접 측정할 수 없으므로 event 명칭을 recognition이나
  detected로 두지 않고 `INTRUDER_VISUAL_OPPORTUNITY_ONSET`으로 정했다.
- LEFT/RIGHT가 기존처럼 45°를 크게 넘지 않도록 최초 상대방위를 ±32°로
  제한했다. FRONT는 0°다.

Lua 변경:

- 활성 version/protocol을 `260804_visual_opportunity_bearing_v33` /
  `audio_task_v33`으로 갱신했다.
- crossing 시작점을 고정 bearing ray와 기존 예상 교차지점의 기하로 다시
  계산했다.
- 추가 500 m path extension을 0으로 바꿔 effective speed가 base speed
  `58 m/s`를 유지하도록 했다.
- spawn 시 beacon/nav/strobe on, landing/taxi off를 시도하고 retire 시 lights
  off를 수행한다.
- 최초 projection이 valid/visible, span 5 px 이상, light write 성공이면
  spawn 직후 `INTRUDER_VISUAL_OPPORTUNITY_ONSET`을 기록한다.
- event detail은 `method=light_assisted_operational_proxy_v1`,
  `operational_proxy=true`, `recognition_claim=false`, light/write 상태와
  projection geometry를 포함한다.
- 기존 `INTRUDER_VISUALLY_DETECTABLE` 20 px event는 보조 진단용으로 남겼다.

Java 변경:

- `XPlaneEventType`에 새 event를 등록했다.
- v33 response baseline/anchor를 opportunity event로 변경했다.
- 뒤늦은 20 px event가 v33 response baseline을 덮어쓰지 않게 했다.
- v33 clean은 opportunity 1회와
  `spawn -> opportunity -> trial end` 순서를 요구하지만 20 px event는
  요구하지 않는다.
- session/batch CSV에 opportunity method, proxy/recognition/light flags,
  projection geometry, IAS, spawn/opportunity/response latency와
  `active_control_at_visual_opportunity`를 추가했다.

Tobii/시각화 변경:

- `analyze_xplane_tobii_session.py`를
  `260804_visual_opportunity_v9`로 갱신했다.
- opportunity-relative external-view windows, first external/AOI gaze/dwell/entry,
  response latency와 aggregate metric을 추가했다.
- v33 opportunity event가 없으면 clean을 거부하며 v32 이하와의 호환성은
  유지한다.
- `visualize_tobii_gaze.py` 기본 anchor를 opportunity event로 변경했다.

검증:

- Java 21 compile: PASS
- `XPlaneAutoEventDetectorSmokeTest`: PASS; v33 opportunity baseline 1회,
  20 px event의 non-overwrite, metadata 및 0.1 s latency 보존 확인
- `XPlaneRandomSpeedTaskAnalysisSmokeTest`: PASS
- Java session/batch export: PASS
- Python 3.10 compile: PASS
- v33 Tobii merge smoke: PASS
- opportunity event 제거 시 `missing INTRUDER_VISUAL_OPPORTUNITY_ONSET`으로
  clean 거부: PASS
- 정적 geometry sweep(90~110 kt 해당 범위): FRONT x=2880,
  LEFT/RIGHT x=1882/3878, 초기 span FRONT 7.9~9.4 px, 측면 12.3~14.7 px

다음 실제 검증:

1. 활성 Lua를 실제 FlyWithLua Scripts 폴더에 복사하고 Reload한다.
2. FRONT/LEFT/RIGHT 실제 생성 위치가 로그와 맞는지 확인한다.
3. beacon/nav/strobe 점멸과 `lights_write_ok=true`를 확인한다.
4. Java/Tobii 동시 실행으로 6-trial validation session을 수집한다.
5. opportunity 기준 response/gaze 결과와 방향별 결과를 분석한다.

## 2026-08-04 v33 physical validation 및 X-Plane ATC crash 분리

First physical run:

- X-Plane session:
  `session_visual_opportunity_v33_20260804_132756`
- Tobii file: `session_xplane_tobii_20260804_132655_gaze.csv`
- trial 1~3 RIGHT/FRONT/LEFT는 모두 clean이었다.
- opportunity x/bearing는 약 `3878/+32°`, `2880/0°`, `1882/-32°`였고
  light write와 새 response anchor가 정상 동작했다.
- trial 3 뒤 washout 중 X-Plane이 crash했다.
- X-Plane log 원문 핵심:
  `ETA is wrong`, `GetGeoPositionAtT`, `ATC_aircraft.cpp:147`.
- 당시 preference는 `_num_other_aircraft 19`, `_num_acf 20`이었으며 crash
  직전 내장 ATC가 p5/p7/p14 route를 dump했다.
- FlyWithLua는 v33을 성공적으로 로드했고 Lua error는 없었다.

Configuration correction:

- AI aircraft를 intruder용 Cessna 172 한 대만 남겼다.
- 확인값: `_num_other_aircraft 1`, `_num_acf 2`.
- JoinFS를 실험 중 비활성화했다.
- 이 구성은 실험 외 traffic 자극과 multiplayer source 충돌을 줄이는
  본실험 운영 규칙으로 채택한다.
...
- X-Plane prefix:
  `logs/xplane/xplane_visual_opportunity_v33_retry_20260804_134141`
- Tobii:
  `logs/tobii/session_xplane_tobii_20260804_134103_gaze.csv`
- 자동 세션 완료 및 crash 미재발
- STATE rows `3083`, INTRUDER rows `3083`, EVENT rows `101`
- trial start/end, spawn, opportunity, baseline, response start/end,
  minimum-distance: `6/6`
- Java strict clean: `6/6 = 100%`
- gaze rows `21199`, valid `17970 = 84.8%`; trial validity `82.5~91.2%`
- schedule: RIGHT, RIGHT, FRONT, FRONT, LEFT, LEFT
- target speed: 110, 105, 90, 100, 95, 90 KIAS
- spawn AGL: 2032, 1837, 1446, 1072, 814, 526 ft
- opportunity span: 13.842, 12.512, 8.820, 8.891, 14.390, 13.816 px
- opportunity-to-response: 12.323, 10.337, 6.166, 4.571, 1.636,
  0.849 s; mean 5.983 s, median 5.369 s
- trigger: pitch, throttle, throttle, pitch, throttle, throttle
- active control at opportunity: trial 1 only
- response before 20 px detectability: trials 3~6 (`4/6`)
- hazard entered: trials 1, 3, 4 (`3/6`)
- trial operational-external rate: 39.9, 43.5, 47.5, 55.7, 56.4, 59.6%

Interpretation:

- physical geometry, lights, v33 opportunity event, Java response anchoring,
  Java/Tobii merge, automatic 6-trial closure는 모두 PASS다.
- 방향이 pair 단위로 묶였고 AGL이 계속 감소했다. opportunity response와
  trial order의 correlation은 `r=-0.986`이므로 이 한 session에서 direction
  effect를 계산하지 않는다.
- clean은 기록 완결성이다. 특히 trial 6의 throttle response `0.849 s`는
  first operational-external `1.692 s`보다 먼저여서 intruder avoidance라고
  확정할 수 없다.
- 다음 구현/분석 단계는 자동 response의 behavior validity를 별도 산출하고,
  speed-task correction, avoidance candidate, ambiguous/no-response로 분류하는
  것이다. dynamic intruder/runway AOI는 그 분류의 보조근거로 추가한다.

## 2026-08-05 v35 bounded speed-stability spawn gate 구현

설계:

- 속도 음성 시작 후 candidate spawn을 기존 `3~10 s`에서 `6~10 s`로 변경했다.
- 목표 `±5 KIAS`, IAS 변화율 `±1 KIAS/s`, `1.5 s` 연속을 속도 안정조건으로
  사용한다. IAS rate는 최소 `0.25 s` 간격으로 갱신한다.
- candidate와 안정조건을 모두 만족하면 즉시 spawn한다.
- 음성 시작 후 `14 s`까지 안정되지 않으면 강제 spawn한다.
- 안정구간이 끊기면 이전 gate-pass 시각도 초기화해 과거의 일시적 안정이
  현재 spawn metadata로 남지 않게 했다.
- 참가자용 minimal display에는 진단정보를 추가하지 않았고 operator overlay에만
  candidate, timeout, speed error/rate, stable 상태를 표시한다.

로그/분석:

- Lua version/protocol/profile:
  `260805_speed_stability_spawn_gate_v35` / `audio_task_v35` /
  `visual_opportunity_proxy_v35`.
- `SCENARIO_SELECTED`와 `INTRUDER_SPAWNED` detail에 gate 설정과 결과를 기록한다.
- Java session/batch summary와 Tobii trial summary에 gate metadata를 추가했다.
- Java/Python timing validity는 안정 생성의 speed error/rate/duration 및
  강제 생성의 14초 도달 여부를 검증한다. timeout metadata가 없으면 invalid다.
- behavior classifier 출력에도 stable/forced/error/rate 필드를 전달한다.
- 원본 STATE/INTRUDER/EVENT CSV 스키마는 변경하지 않았다.

기준 선택 근거:

- v33 retry 6-trial 로그를 읽기 전용으로 근사 재생했다.
- `±3 KIAS`, rate `±1 KIAS/s`, 1.5초에서는 timeout 예상 `3/6`이었다.
- `±5 KIAS`, 같은 rate/duration에서는 timeout 예상 `1/6`이었고, 해당 trial은
  timeout 시점에도 목표와 약 8 KIAS 차이였다.
- 기존 분석도 `within_5_kias`를 사용하므로 최종 오차 허용범위를 ±5 KIAS로
  맞췄다. 이 back-test는 실제 v35 validation을 대체하지 않는다.

검증:

- Java 21 compile: PASS
- `XPlaneRandomSpeedTaskAnalysisSmokeTest`: PASS; v35 안정 생성, forced timeout,
  조기 forced 거부, timeout 누락 거부, 오차범위 초과 거부 포함
- `XPlaneAutoEventDetectorSmokeTest`: PASS
- Python 3.10 compile: PASS
- `analyze_xplane_tobii_session_smoke_test.py`: PASS
- `classify_xplane_tobii_behavior_smoke_test.py`: PASS
- batch trial export: header/data `117/117`, gate metadata와 timing valid 확인
- Lua 기준본/snapshot/실제 Scripts 실행본 SHA-256 일치:
  `D9604131AF8A7EDB63EE906221127860287CCA458D09FE24027151421C4B132B`
- 로컬 `lua`/`luac`: PATH에 없음
- 실제 X-Plane reload: PASS; `Log.txt`에
  `script loaded: 260805_speed_stability_spawn_gate_v35`, Lua 오류·quarantine 없음

다음 실제 절차:

1. Java receiver와 Tobii logger를 시작한다.
2. 저장 시나리오에서 자동 6-trial 한 세션을 수행한다.
3. 종료 후 stable spawn 수, forced-timeout 수, 실제 audio-to-spawn 범위와
   behavior 분류를 분석한다.
4. forced 비율이 반복 세션에서도 과도할 때만 tolerance/timeout을 다시 조정한다.

## 2026-08-05 v35 first physical validation 분석

Run:

- session: `session_speed_gate_v35_validation_20260805_154919`
- X-Plane prefix:
  `logs/xplane/xplane_speed_gate_v35_validation_20260805_154919`
- gaze: `logs/tobii/session_xplane_tobii_20260805_154831_gaze.csv`
- final rows: STATE `3049`, INTRUDER `3049`, EVENT `103`
- clean shutdown: `RECEIVER_STOP`, `SESSION_STOP`

Recording/runtime:

- v35 reload, automatic completion, 6 reset/start/end, 6 spawn: PASS
- Java strict clean: `6/6`
- directions: FRONT, LEFT, RIGHT, LEFT, FRONT, RIGHT
- speed targets: 95, 90, 105, 110, 95, 100 KIAS
- hazard detect/clear: `4/4`
- session minimum horizontal/vertical-at-min: `4.713/10.220 m`
- all v35 `task_audio_spawn_timing_valid=true`

Gate results:

| trial | target | candidate s | actual s | error kt | rate kt/s | stable duration s | result |
|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 95 | 9.822 | 14.003 | +0.297 | -0.983 | 0.110 | forced |
| 2 | 90 | 9.920 | 11.290 | +3.938 | -0.726 | 1.506 | stable |
| 3 | 105 | 6.364 | 14.014 | -6.499 | +1.412 | 0.000 | forced |
| 4 | 110 | 8.375 | 9.770 | -3.686 | +0.903 | 1.514 | stable |
| 5 | 95 | 9.016 | 14.008 | +0.299 | -0.672 | 0.838 | forced |
| 6 | 100 | 9.579 | 9.595 | -3.853 | -0.024 | 9.343 | stable |

판정:

- trial 3은 실제 목표속도 미완료라 forced가 타당하다.
- trial 1과 5는 timeout 시 목표범위 중심에 가까웠지만 raw IAS-rate 조건이
  연속 안정시간을 reset해 forced가 됐다.
- rate 계산창 0.25/0.5/0.75/1.0초 sensitivity에서는 모두 forced 3/6으로
  단순 smoothing-window 변경만으로 해결되지 않았다.
- rate tolerance ±1.25는 후향상 forced 1/6이지만 단일 pilot fitting이 될 수 있다.
- `±5 KIAS를 1.5초 연속 유지`만 필수 gate로 두면 후향상 stable 5/6이고
  trial 3만 forced다. IAS rate를 진단값으로 계속 기록하는 v36 후보가 더
  단순하고 논문에서 설명하기 쉽다.

Tobii/behavior:

- gaze rows/valid: `29030 / 21905 = 75.5%`
- trial validity: `88.5, 79.2, 85.9, 87.1, 92.6, 81.2%`
- measured/inferred external episodes: `38/51`
- opportunity-to-response:
  `10.058, 0.876, 0.752, 2.831, 0.496, 7.264 s`
- behavior: AMBIGUOUS trial 1,2,3,5; AVOIDANCE_CANDIDATE trial 4;
  SPEED_TASK_CORRECTION trial 6
- stable trials 안에서도 dual speed/avoidance와 speed-only가 존재하므로 gate는
  behavior intent 분류를 대체하지 않는다.

Outputs:

- `build_atc_tmp/xplane_speed_gate_v35_validation_20260805_154919_gaze_aoi.csv`
- `build_atc_tmp/xplane_speed_gate_v35_validation_20260805_154919_external_episodes.csv`
- `build_atc_tmp/xplane_speed_gate_v35_validation_20260805_154919_trial_gaze_summary.csv`
- `build_atc_tmp/xplane_speed_gate_v35_validation_20260805_154919_trial_aggregate_summary.csv`
- `build_atc_tmp/xplane_speed_gate_v35_validation_20260805_154919_trial_behavior_summary.csv`

현재 상태:

- v35 functional/runtime/data validation: PASS
- current strict speed+rate gate freeze: 보류
- 권장 다음 단계: IAS rate를 필수조건에서 제외하고 diagnostic으로 유지하는
  v36을 구현한 뒤 동일 조건 6-trial 재검증

## 2026-08-05 v36 target-band dwell spawn gate 구현

Lua:

- version/protocol/profile:
  `260805_target_band_dwell_spawn_gate_v36` / `audio_task_v36` /
  `visual_opportunity_proxy_v36`
- gate method: `bounded_speed_band_dwell_v2`
- target band `±5 KIAS`, dwell `1.5 s`, candidate `6~10 s`, timeout `14 s`
- `within_gate`에서 IAS-rate-valid와 `±1 KIAS/s` 조건을 제거했다.
- IAS rate 계산은 `0.25 s` 간격으로 계속 수행하고 spawn detail에 기록한다.
- scenario/spawn detail에 `speed_rate_diagnostic_only=true`,
  `speed_gate_condition=target_error_band_dwell`을 추가했다.
- 기존 UDP/CSV packet schema는 변경하지 않았다.

Java/Python:

- `audio_task_v36`을 audio ordering과 visual-opportunity clean protocol에 추가했다.
- v36 timing-valid stable branch는 error `±5.05 KIAS`와 dwell `>=1.49 s`를
  검증하며 rate 값/validity를 요구하지 않는다.
- v35 timing-valid branch는 기존 rate-valid, rate `±1.05 KIAS/s` 조건을
  그대로 유지한다.
- Python analyzer version:
  `260805_target_band_dwell_spawn_gate_v11`.
- batch/classifier output schema는 그대로이며 rate 진단 열도 유지된다.

검증:

- Java 21 compile: PASS
- `XPlaneRandomSpeedTaskAnalysisSmokeTest`: PASS; v36 rate diagnostic-only,
  forced timeout, early timeout 거부, metadata 누락/오차 초과 거부, v35 rate
  backward rule 포함
- `XPlaneAutoEventDetectorSmokeTest`: PASS; v36 opportunity anchor
- Python compile, analyzer timing smoke, behavior classifier smoke: PASS
- v36 synthetic batch: clean `1/1`, header/data `117/117`, rate `-2.000`을
  보존하면서 timing-valid true
- v35 physical session Java regression: `6/6 clean`, timing-valid `6/6`
- v35 physical session Python regression: gaze `29030`, valid `21905 (75.5%)`,
  clean/timing-valid `6/6`
- Lua format placeholders: scenario `9`, spawn `42`; `git diff --check` PASS
- 저장소 기준본, v36 snapshot, 실제 X-Plane Scripts 실행본 SHA-256:
  `1D015BF392ADF09ECDFB4AA32275B804A3F0D76AC58A8C50D7829C2B850AA3BA`
- 실제 Scripts 폴더에는 `FLYWITHLUA_STUDY_INTEGRATED.lua` 한 개만 존재
- local lua/luac unavailable; 실제 FlyWithLua reload PASS
- X-Plane `Log.txt`: `script loaded: 260805_target_band_dwell_spawn_gate_v36`,
  `All script files loaded successfully`, quarantine Lua `0`

다음 단계:

1. v36 gate parameter를 현재 freeze candidate로 유지한다.
2. 이후 반복/참가자 session에서도 stable/forced status를 별도로 보존한다.
3. 본실험 분석계획에 forced 포함/제외 sensitivity를 명시한다.

## 2026-08-05 v36 first physical validation 분석

Run:

- session: `session_speed_gate_v36_validation_20260805_162420`
- X-Plane prefix:
  `logs/xplane/xplane_speed_gate_v36_validation_20260805_162420`
- gaze: `logs/tobii/session_xplane_tobii_20260805_162344_gaze.csv`
- final rows: STATE `2841`, INTRUDER `2841`, EVENT `105`, gaze `25185`
- normal shutdown: `RECEIVER_STOP=1`, `SESSION_STOP=1`

Recording/runtime:

- reset/start/end/spawn/opportunity/detectability/response: 각 `6`
- Java strict clean: `6/6`
- task audio/spawn timing-valid: `6/6`
- direction schedule: `LEFT|FRONT|RIGHT|LEFT|FRONT|RIGHT`
- speed schedule: `105|95|100|90|90|110 KIAS`
- hazard detect/clear: `5/5`; trial 4는 hazard 진입 없음

Spawn gate:

- stable: trial `1,2,3,5` = `4/6`
- forced at 14 s: trial `4,6` = `2/6`
- stable spawn errors: `-0.373,+1.498,-3.395,-1.819 KIAS`
- forced spawn errors: `+6.893,-7.835 KIAS`
- stable trial 2/3 rate diagnostics: `-1.020,+1.199 KIAS/s`
- v35처럼 error band 안인데 raw rate 때문에 forced가 된 사례: `0`

Gaze/response:

- raw valid: `20748/25185 = 82.4%`
- trial valid: `84.6~92.6%`, equal-weight mean `89.2%`
- measured/inferred side episodes: `58/25`
- opportunity-to-response:
  `1.956,5.451,5.717,3.394,6.461,3.435 s`; mean `4.402 s`
- operational-external before response: `6/6`
- initial span `8.556~14.259 px`라 5 px opportunity는 spawn과 동시 `6/6`
- 20 px detectability: spawn 후 `5.160~11.283 s`; post-detectability response는
  trial 3 한 건(`+0.557 s`)

Behavior evidence:

- AMBIGUOUS: trial `1,2,3,4`
- AVOIDANCE_CANDIDATE: trial `5,6`
- trial 4는 dual speed/avoidance evidence라 intent를 확정하지 않는다.
- 모든 operational-external과 avoidance-candidate는 intruder acquisition 또는
  pilot intent의 확정값으로 해석하지 않는다.

v35/v36 comparison:

- v35: stable/forced `3/3`, raw gaze valid `75.5%`, trial-average valid `85.7%`,
  opportunity-response mean `3.713 s`, behavior `4 ambiguous/1 avoidance/1 speed`
- v36: stable/forced `4/2`, raw gaze valid `82.4%`, trial-average valid `89.2%`,
  opportunity-response mean `4.402 s`, behavior `4 ambiguous/2 avoidance`
- 두 session은 같은 pilot의 단일 반복이므로 latency/behavior 차이를 gate의
  인과효과로 해석하지 않는다. gate 판정의 논리적 일치 여부만 비교한다.

Outputs:

- `logs/xplane/xplane_batch_analysis_20260805_163215_sessions.csv`
- `logs/xplane/xplane_batch_analysis_20260805_163215_trials.csv`
- `logs/xplane/xplane_batch_analysis_20260805_163215_clean_trials.csv`
- `build_atc_tmp/xplane_speed_gate_v36_validation_20260805_162420_gaze_aoi.csv`
- `build_atc_tmp/xplane_speed_gate_v36_validation_20260805_162420_external_episodes.csv`
- `build_atc_tmp/xplane_speed_gate_v36_validation_20260805_162420_trial_gaze_summary.csv`
- `build_atc_tmp/xplane_speed_gate_v36_validation_20260805_162420_trial_aggregate_summary.csv`
- `build_atc_tmp/xplane_speed_gate_v36_validation_20260805_162420_trial_behavior_summary.csv`

결론:

- v36 target-band dwell engineering validation: PASS
- current gate freeze candidate: 채택 권장
- 본실험 분석: recording clean과 stable/forced를 분리하고 forced 포함/제외
  sensitivity를 유지
- 남은 연구 한계: visual opportunity가 spawn과 동시이며 behavior intent가
  다수 ambiguous라는 점

## 2026-08-06 그룹미팅 피드백 정리

교수 피드백:

- 조종사가 속도를 조절했는지 intruder를 회피했는지 이유를 반드시 구분할
  필요는 없다.
- 화면상 intruder 위치와 크기를 계산해 표준화된 발견 가능 시점 이후
  반응시간을 측정한다.
- 외부환경 시선은 격자를 이용해 공간적으로 구분한다.

해석 및 결정:

- behavior classifier는 삭제하지 않지만 primary label이 아니라 diagnostic으로
  내린다.
- grid는 spatial method이고 `RUNWAY/INTRUDER/OTHER`는 semantic category다.
  두 층을 함께 저장한다.
- semantic category는 `RUNWAY_GUIDANCE`, `INTRUDER_SEARCH_OR_TRACKING`,
  `OTHER_EXTERNAL`, `UNRESOLVED`를 유지한다. residual class를 없애고 모든
  정면 외부시선을 runway/intruder로 강제 분류하지 않는다.
- 좌우 measured gaze는 spawn 이후 실제 intruder 방향과 일치할 때
  `INTRUDER_SEARCH_PROXY`로 해석한다. inferred direction unknown과 반대편
  side gaze는 별도로 보존한다.
- 정면은 grid heat/distribution 외에 projected intruder dynamic AOI와 runway
  AOI가 필요하다. 작은 moving object는 grid cell만으로 gaze-on-object를
  확정하지 않는다.
- 현재 코드의 beacon/nav/strobe는 ON이며, 다음 revision에서 OFF로 바꾸고
  `light_assisted` profile도 geometry-based profile로 갱신해야 한다.
- intruder 생성은 fixed distance를 직접 늘리는 대신 목표 initial span과
  spawn-to-opportunity 시간을 정한 뒤 방향별 거리·속도를 계산한다.

다음 작업 순서:

1. lights OFF와 geometry-based opportunity의 parameter sweep을 구현한다.
2. 전체 세션이 아닌 FRONT/LEFT/RIGHT 짧은 calibration으로 initial span,
   opportunity threshold, delay, speed cap을 확인한다.
3. fixed grid spatial fields와 projected intruder dynamic AOI를 gaze analyzer에
   추가한다.
4. runway AOI와 `OTHER/UNRESOLVED` residual mapping을 추가한다.
5. 새 기준으로 자동 6-trial Java/Tobii validation을 수행한다.

## 2026-08-11 v37 lights-off geometry calibration 구현

목적:

- v36의 점멸등과 spawn 동시 5 px opportunity를 제거한다.
- 화면상 intruder 위치·크기를 이용한 표준화된 반응 기준을 실제 X-Plane에서
  교정할 수 있게 한다.

수정:

- `FLYWITHLUA_STUDY_INTEGRATED.lua`
  - version/protocol/profile을 v37로 갱신했다.
  - crossing과 head-on intruder lights를 OFF로 고정했다.
  - crossing base speed를 `58 -> 65 m/s`로 조정했다.
  - `study_record_geometry_visual_opportunity()`를 추가했다.
  - 예상 span이 최초 `20 px`에 도달할 때
    `INTRUDER_VISUAL_OPPORTUNITY_ONSET`을 기록한다.
  - spawn 시 5 px/light-assisted event와 gate-failure `MANUAL_NOTE`는 제거했다.
  - spawn/opportunity detail에 `light_assisted=false`, `lights=off`, 화면좌표,
    거리, 초기 span 및 교정 목표 범위를 기록한다.
- `XPlaneSessionAnalysisMain.java`
  - `audio_task_v37`을 visual-opportunity protocol과 v36형 target-band dwell
    timing validation에 추가했다.
- `analyze_xplane_tobii_session.py`
  - analyzer version을 `260811_geometry_opportunity_calibration_v12`로 갱신하고
    v37 clean/timing/response-anchor 규칙을 추가했다.
- Java/Python smoke는 v37 지연 opportunity 사례를 검사하도록 갱신했고,
  v35/v36 후방 호환 assertion을 유지했다.

검증:

- Java compile은 `AtcServerBridgeClient.java`와 `XPlane*.java`를 Java 21로 함께
  컴파일해 PASS했다. `XPlane*.java`만 컴파일하면 기존 ATC 의존 클래스 누락으로
  실패하므로 현재 작업환경에서는 해당 의존 파일을 함께 포함한다.
- Java auto-event smoke PASS
- Java v37 random-speed/session-analysis smoke PASS
- Python analyzer/classifier compile 및 smoke PASS
- 실제 v36 session
  `session_speed_gate_v36_validation_20260805_162420` 후방 호환 재분석 PASS:
  Java `6/6 clean`, Python raw gaze `20748/25185 = 82.4%`
- Lua 기준본과 snapshot SHA-256:
  `002B1235B3C2BE6A7833435D6CCF91F876874DF7A8A575D06794705D310328F7`
- 실제 X-Plane FlyWithLua Scripts 실행본 배포 완료, 동일 hash 확인

판정:

- v37은 최종 실험 파라미터 동결본이 아니라 physical calibration build다.
- `20 px` event는 실제 발견시점이 아니라 geometry-defined visual opportunity
  proxy다.
- initial span 목표 `7~13 px`, spawn-to-opportunity 목표 `5~11 s`, base speed
  `65 m/s`는 metadata로 측정하며 아직 trial 생성의 강제 합격조건은 아니다.
- 다음 작업은 X-Plane reload 후 짧은 FRONT/LEFT/RIGHT 교정 세션을 수행해
  조명 OFF와 방향별 geometry를 확인하는 것이다.

Runtime reload 확인:

- X-Plane에서 FlyWithLua `Reload all Lua script files` 수행
- `script loaded: 260811_geometry_opportunity_calibration_v37`: 확인
- `All script files loaded successfully`: 확인
- Lua stop/stack traceback: 없음
- Scripts 활성 Lua: `1`개
- `Scripts (Quarantine)` Lua: `0`개
- 현재 실행 중 Java 프로세스는 IntelliJ build server와 IDE뿐이며
  `XPlaneReceiverMain`은 없다.
- 다음 즉시 작업은 Tobii logger와 새 timestamp의 Java receiver를 시작하고
  v37 교정 session을 수집하는 것이다.

## 2026-08-11 v37 first physical calibration 분석

Run:

- session: `session_geometry_calibration_v37_20260811_144334`
- X-Plane prefix:
  `logs/xplane/xplane_geometry_calibration_v37_20260811_144334`
- gaze: `logs/tobii/session_xplane_tobii_20260811_144244_gaze.csv`
- final rows: STATE `3209`, INTRUDER `3209`, EVENT `101`, gaze `34432`
- `RECEIVER_STOP=1`, `SESSION_STOP=1`, Java/Tobii process 정상 종료

Recording:

- trial: `6/6 clean`
- direction: `RIGHT|FRONT|LEFT|RIGHT|FRONT|LEFT`
- spawn/opportunity/detectability/response-start/response-end/end: 각 `6`
- profile/method: `visual_opportunity_proxy_v37` /
  `geometry_span_operational_proxy_v2`
- `lights=off`, `lights_write_ok=true`, `recognition_claim=false`: `6/6`
- user-visible blinking light의 실제 소멸 여부: 참가자 확인 대기

Geometry:

| Trial | Direction | Initial span px | Initial range m | Spawn→20 px s | 20 px range m |
|---:|---|---:|---:|---:|---:|
| 1 | RIGHT | 13.384 | 1821.507 | 6.124 | 1223.279 |
| 2 | FRONT | 8.021 | 2195.959 | 10.949 | 889.434 |
| 3 | LEFT | 12.087 | 2009.034 | 7.979 | 1218.647 |
| 4 | RIGHT | 13.191 | 1855.783 | 6.308 | 1231.878 |
| 5 | FRONT | 8.547 | 2060.072 | 10.131 | 879.032 |
| 6 | LEFT | 13.233 | 1850.845 | 6.519 | 1224.480 |

- delay target `5~11 s`: `6/6`
- provisional initial span target `7~13 px`: `3/6`; 초과값은
  `13.191,13.233,13.384 px`
- 실제 initial span range: `8.021~13.384 px`
- direction mean initial span/delay: FRONT `8.284/10.540`, LEFT
  `12.660/7.249`, RIGHT `13.288/6.216`
- geometry 판정: `65 m/s + 20 px` engineering candidate PASS

Response/gaze:

- opportunity-to-response: `3.185,0.347,2.852,1.860,2.011,0.543 s`
- mean/median: `1.800/1.936 s`
- post-opportunity response: `6/6`
- trial 2 active control at opportunity: true; `0.347 s` 해석 주의
- raw gaze valid: `24019/34432 = 69.8%`
- trial gaze valid: `85.4~92.7%`, equal-weight mean 약 `88.1%`
- measured/inferred external episodes: `48/43`

Speed gate failure:

- stable spawn: `0/6`
- forced timeout spawn: `6/6`
- spawn error KIAS:
  `+5.967,-11.999,+6.418,-5.764,-5.594,+6.295`
- v37 timing-valid `6/6`은 timeout branch가 계약대로 동작했다는 뜻일 뿐 목표속도
  과제 성공을 뜻하지 않는다.
- 다음 권장안은 tolerance/timeout 단순 확대가 아니라 audio 시점 현재 IAS
  부근에서 5-knot 단위 목표를 고르는 adaptive feasible target이다.

Generated outputs:

- `build_atc_tmp/xplane_geometry_calibration_v37_20260811_144334_gaze_aoi.csv`
- `build_atc_tmp/xplane_geometry_calibration_v37_20260811_144334_external_episodes.csv`
- `build_atc_tmp/xplane_geometry_calibration_v37_20260811_144334_trial_gaze_summary.csv`
- `build_atc_tmp/xplane_geometry_calibration_v37_20260811_144334_trial_aggregate_summary.csv`
- `build_atc_tmp/xplane_geometry_calibration_v37_20260811_144334_geometry_summary.csv`
- `build_atc_tmp/xplane_geometry_calibration_v37_20260811_144334_geometry_direction_summary.csv`

## 2026-08-28 연구실 기술 파일럿 및 최신 reference 분석

수집 구성:

- 연구실 참가자 시도 `7명`; complete `6명`, partial/water impact `1명`
- primary complete trials `36`; FRONT/LEFT/RIGHT 각 `12`
- partial은 사고 전 완료된 `4` trial만 보존하고 zero-impute하지 않음
- 보고서 participant id는 `P01~P07`; 실명 원본 CSV는 Git에 포함하지 않음

Complete-six 결과:

- trial 동일가중 gaze validity 약 `96.3%`
- response detection `31/36 = 86.1%`
- detected opportunity-to-response mean/median `3.135/2.138 s`
- full-trial eligible bins `6077`, DTMC transitions `5942`, self `4745`,
  switches `1197`, self rate `79.9%`
- participant-equal switch frequency 약 `59.0/min`
- operational-external participant range `4.0~71.7%`
- speed stable/forced `16/20`; target 110 KIAS `7/7` forced
- opportunity 당시 active control은 `9/36`; active trial latency와 inactive trial
  latency를 분리하고 매우 짧은 값을 실제 인지·새 회피반응으로 단정하지 않음

Group phase pattern:

- BEFORE_SPAWN self `79.9%`, switches `58.9/min`
- SPAWN_TO_OPPORTUNITY self `82.0%`, switches `51.8/min`
- AFTER_OPPORTUNITY self `78.2%`, switches `62.7/min`
- 이 변화는 descriptive technical-pilot 결과이며 inferential effect로 표현하지 않음

Latest experienced reference:

- X-Plane: `xplane_main_KIM_SEOKHO_20260826_164645`
- gaze: `session_xplane_tobii_KIM_SEOKHO_20260826_164659_gaze.csv`
- clean `6/6`, raw gaze valid `88.5%`, trial mean valid `91.5%`
- operational-external `48.4%`
- opportunity-response 전체 mean/median `2.467/2.442 s`
- active-control 제외 `n=3`, mean `3.834 s`
- instrument/panel-other/external bins `43.3/2.6/54.2%`
- DTMC self `74.0%`, switches `74.4/min`
- minimum horizontal distance mean/median `35.0/25.8 m`
- speed stable/forced `3/3`, active at opportunity `3/6`
- 8월 12일 self와 broad state share 및 Matrix가 유사해 scan-pattern repeatability를
  보였지만 두 세션을 단순 평균하지 않음

논문/공모전 방향:

- APISAT은 pilot superiority가 아니라 synchronized multimodal framework와
  AOI transition analysis의 technical pilot validation으로 작성
- KAI 항공우주논문상은 `미래SW`로 분류하고 MUH/마린온 simulator adapter 및
  교관 디브리핑 지원을 후속 산업 적용으로 제시
- 실제 마린온 적용 검증 완료로 표현하지 않으며 기체 외형만 바꾼 중복논문을 만들지 않음
- IRB/면제 판단과 APISAT/KAI 중복게재 규정을 제출 전에 확인

## 2026-08-28 APISAT full-paper 초안 생성

- 입력: `2026 APISAT/FullPaperTemplate_APISAT-2026.docx`, 기존 최종 초록,
  최신 기술 파일럿·DTMC·visual-opportunity 분석 결과
- 생성: `2026 APISAT/APISAT-2026_FullPaper_Draft_260828.docx`
- 검토용 PDF: `2026 APISAT/APISAT-2026_FullPaper_Draft_260828.pdf`
- 편집 원본: `2026 APISAT/APISAT-2026_FullPaper_Draft_260828.md`
- 재생성 도구: `tools/docs/create_apisat_fullpaper_draft.ps1`
- 결과: 7 pages, 3,998 words, abstract 163 words, figures 4, data tables 3
- Word/PDF 실물 검수에서 그림 줄높이 clipping과 수식 번호 줄바꿈을 발견하여 수정했고,
  최종 PDF 7쪽 모두 재렌더링하여 그림·표·2단 편집·참고문헌 표시를 확인했다.
...
## 2026-09-08 문서 최신화 및 Git 반영 준비

- `Agents.md`, `Project_Context.md`, `Worklog.md`에 APISAT full-paper 초안과 해병항공 워크숍 발표자료까지의 작업 상태를 정리했다.
- 해병항공 워크숍 폴더에 발표 내용·기술 검증 수치·군 활용 제안·한계·재생성 방법을 설명하는 `README.md`를 추가했다.
- 발표자료 생성에 사용한 AOI cockpit overlay와 complete-six DTMC 그림을 워크숍 폴더 `assets`로 복사하고 원본과 SHA-256이 일치함을 확인했다.
- `create_marine_aviation_workshop_presentation.ps1`의 그림 경로를 추적 가능한 `assets` 경로로 변경했다.
- 이번 Git 반영에서는 원본 실험 로그와 CSV, `build_atc_tmp`, 로컬 `2026 APISAT/` 자료, `.idea/misc.xml` 개인 설정을 제외한다.
