...
local STUDY_SCRIPT_VERSION = "260805_speed_stability_spawn_gate_v35"
local PARTICIPANT_NOTICE_DURATION_S = 2.5
local TASK_AUDIO_PROTOCOL = "audio_task_v35"
...
local SPEED_GATE_RATE_TOLERANCE_KIAS_S = 1.0
local SPEED_GATE_STABILITY_DURATION_S = 1.5
local SPEED_GATE_RATE_SAMPLE_INTERVAL_S = 0.25
...
local INTRUDER_VISUAL_OPPORTUNITY_MIN_SPAN_PX = 5.0
...
function study_update_crossing_speed_gate()
    ...
    if rate_sample_duration_s >= SPEED_GATE_RATE_SAMPLE_INTERVAL_S then
    ...
    local within_gate = crossing_speed_rate_valid
        and math.abs(crossing_speed_error_kias) <= SPEED_GATE_ERROR_TOLERANCE_KIAS
        and math.abs(crossing_speed_rate_kias_s) <= SPEED_GATE_RATE_TOLERANCE_KIAS_S
...
function study_start_intruder_headon()
    ...
    study_set_intruder_lights(true)
...
function study_arm_random_crossing(audio_start_sim_time_s)
    ...
    study_record_event(
        ...
        study_trial_detail(
            string.format(
                "scenario=random_final_approach_crossing;delay_anchor=task_audio_start;spawn_gate_method=bounded_speed_stability_v1;planned_audio_to_spawn_s=%.3f;candidate_audio_to_spawn_s=%.3f;spawn_timeout_audio_to_spawn_s=%.3f;speed_error_tolerance_kias=%.3f;speed_rate_tolerance_kias_s=%.3f;speed_stability_required_s=%.3f;speed_rate_sample_interval_s=%.3f;audio_start_sim_time_s=%.3f;crossing_trigger_time_s=%.3f;crossing_timeout_time_s=%.3f",
                crossing_planned_audio_to_spawn_s,
                crossing_planned_audio_to_spawn_s,
                SPEED_GATE_TIMEOUT_S,
                SPEED_GATE_ERROR_TOLERANCE_KIAS,
                SPEED_GATE_RATE_TOLERANCE_KIAS_S,
                SPEED_GATE_STABILITY_DURATION_S,
                SPEED_GATE_RATE_SAMPLE_INTERVAL_S,
...
function study_start_intruder_crossing()
    ...
    crossing_lights_write_ok = study_set_intruder_lights(true)
    ...
    crossing_spawn_event_detail = study_trial_detail(
        string.format(
            ...
            STUDY_DISPLAY_HEIGHT_PX,
            STUDY_HORIZONTAL_FOV_DEG,
            INTRUDER_VISUAL_OPPORTUNITY_MIN_SPAN_PX,
            INTRUDER_VISUAL_DETECTABILITY_THRESHOLD_PX,
    ...
    if initial_projection.valid
        and initial_projection.visible
        and initial_projection.span_px >= INTRUDER_VISUAL_OPPORTUNITY_MIN_SPAN_PX
        and crossing_lights_write_ok then
        crossing_visual_opportunity_recorded = true
        study_record_event(
            "INTRUDER_VISUAL_OPPORTUNITY_ONSET",
            study_trial_detail(
                string.format(
                    "method=light_assisted_operational_proxy_v1;operational_proxy=true;recognition_claim=false;lights=beacon_nav_strobe;lights_write_ok=%s;minimum_span_px=%.3f;estimated_span_px=%.3f;screen_x_px=%.3f;screen_y_px=%.3f;slant_distance_m=%.3f;horizontal_distance_m=%.3f;relative_bearing_deg=%.3f;desired_relative_bearing_deg=%.3f;relative_elevation_deg=%.3f;display_width_px=%.0f;display_height_px=%.0f;horizontal_fov_deg=%.3f;spawn_to_opportunity_s=0.000",
                    tostring(crossing_lights_write_ok),
                    INTRUDER_VISUAL_OPPORTUNITY_MIN_SPAN_PX,
                    initial_projection.span_px,
                    initial_projection.screen_x_px,
                    initial_projection.screen_y_px,
                    initial_projection.slant_distance_m,
                    initial_projection.horizontal_distance_m,
                    initial_projection.relative_bearing_deg,
                    crossing_desired_initial_bearing_deg,
                    initial_projection.relative_elevation_deg,
                    STUDY_DISPLAY_WIDTH_PX,
        ...
        study_record_event(
            ...
            study_trial_detail(
                string.format(
                    "visual_opportunity_gate_failed;valid=%s;visible=%s;span_px=%.3f;minimum_span_px=%.3f;lights_write_ok=%s;relative_bearing_deg=%.3f",
                    tostring(initial_projection.valid),
                    tostring(initial_projection.visible),
                    initial_projection.span_px,
                    INTRUDER_VISUAL_OPPORTUNITY_MIN_SPAN_PX,
